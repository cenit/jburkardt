...
//
    else
    {
      ...
      for ( j = imaxp1; j <= k; j++ )
      {
        rowmax = r8_max ( rowmax, fabs ( a[imax-1+(j-1)*lda] ) );
      }

      if ( imax != 1 )
      {
        jmax = idamax ( imax-1, a+0+(imax-1)*lda, 1 );
        rowmax = r8_max ( rowmax, fabs ( a[jmax-1+(imax-1)*lda] ) );
      }
...
//
    else
    {
      ...
      for ( j = imaxp1; j <= k; j++ )
      {
        rowmax = r8_max ( rowmax, fabs ( ap[imj-1] ) );
        imj = imj + j;
      }
      ...
      if ( imax != 1 )
      {
        ...
        jmim = jmax + im;
        rowmax = r8_max ( rowmax, fabs ( ap[jmim-1] ) );
      }
