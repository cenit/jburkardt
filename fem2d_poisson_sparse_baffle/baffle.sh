...
#
g++ ~/libcpp/$ARCH/fem2d_poisson_sparse.o baffle.o
if [ $? -ne 0 ]; then
  echo "Errors linking and loading fem2d_poisson_sparse.o + baffle.o"
  exit
fi
rm baffle.o
#
chmod ugo+x a.out
mv a.out fem2d_poisson_sparse_baffle
./fem2d_poisson_sparse_baffle baffle > baffle_output.txt
if [ $? -ne 0 ]; then
  echo "Errors running baffle."
  exit
fi
rm fem2d_poisson_sparse_baffle
#
