...
#
g++ ~/libcpp/$ARCH/fem2d_poisson_cg.o lake.o
if [ $? -ne 0 ]; then
  echo "Errors linking and loading fem2d_poisson_cg.o + lake.o"
  exit
fi
rm lake.o
#
chmod ugo+x a.out
mv a.out fem2d_poisson_cg_lake
./fem2d_poisson_cg_lake lake > lake_output.txt
if [ $? -ne 0 ]; then
  echo "Errors running lake."
  exit
fi
rm fem2d_poisson_cg_lake
#
