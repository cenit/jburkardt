R8S3\
SLAP Triad Matrix Utilities {#r8s3-slap-triad-matrix-utilities align="center"}
===========================

------------------------------------------------------------------------

**R8S3** is a C++ library which carries out some basic linear algebra
operations on R8S3 matrices (real 64 bit precision SLAP Triad (I,J,AIJ)
format).
...
**R8S3** is available in [a C version](../../c_src/r8s3/r8s3.html) and
[a C++ version](../../cpp_src/r8s3/r8s3.html) and [a FORTRAN90
version](../../f_src/r8s3/r8s3.html) and [a MATLAB
version](../../m_src/r8s3/r8s3.html) and [a Python
version.](../../py_src/r8s3/r8s3.html)
...
-   [r8s3.cpp](r8s3.cpp), the source code.
-   [r8s3.hpp](r8s3.hpp), the source code.

### Examples and Tests: {#examples-and-tests align="center"}

-   [r8s3\_prb.cpp](r8s3_prb.cpp), a sample calling program.
-   [r8s3\_prb\_output.txt](r8s3_prb_output.txt), the output file.

### List of Routines: {#list-of-routines align="center"}

-   **FILE\_DELETE** deletes a named file if it exists.
-   **FILE\_EXIST** reports whether a file exists.
-   **FILE\_IS\_OPEN** reports whether a file (specified by filename) is
    open.
...
-   **R8S3\_DIAGONAL** reorders an R8S3 matrix so diagonal entries are
    first.
-   **R8S3\_DIF2** sets up an R8S3 second difference matrix.
-   **R8S3\_INDICATOR** sets up an R8S3 indicator matrix.
-   **R8S3\_JAC\_SL** solves an R8S3 system using Jacobi iteration.
-   **R8S3\_MTV** multiplies an R8VEC times an R8S3 matrix.
-   **R8S3\_MV** multiplies an R8S3 matrix by an R8VEC.
-   **R8S3\_PRINT** prints an R8S3 matrix.
-   **R8S3\_PRINT\_SOME** prints some of an R8S3 matrix.
-   **R8S3\_READ** reads a square R8S3 matrix from a file.
-   **R8S3\_READ\_SIZE** reads the size of a square R8S3 matrix from a
    file.
-   **R8S3\_RES** computes the residual R = B-A\*X for R8S3 matrices.
-   **R8S3\_TO\_R8GE** copies an R8S3 matrix to an R8GE matrix.
-   **R8S3\_WRITE** writes a square R8S3 matrix to a file.
-   **R8S3\_ZEROS** zeroes an R8S3 matrix.
...
*Last revised on 18 September 2015.*
