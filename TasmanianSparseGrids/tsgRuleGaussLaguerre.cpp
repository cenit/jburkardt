...
#ifndef __TASMANIAN_SPARSE_GRID_RULE_GGL_CPP
#define __TASMANIAN_SPARSE_GRID_RULE_GGL_CPP

#include "tsgRuleGaussLaguerre.hpp"

namespace TasGrid{

RuleGaussLaguerre::RuleGaussLaguerre( const int level, double walpha ) : max_level(level), tol(NUM_TOL), alpha(walpha), OneDRule(){
        int total_points = 0;
...
RuleGaussLaguerre::~RuleGaussLaguerre(){
...
int RuleGaussLaguerre::getMaxLevel() const{ return max_level; }

TypeOneDRule RuleGaussLaguerre::getType() const{
        return rule_gaussgegenbauer;
}

int RuleGaussLaguerre::getNumPoints( int level ) const{
        return level+1;
}

int RuleGaussLaguerre::getBasisLevel( int level ) const{
        return 2*level;
}

void RuleGaussLaguerre::getPoints( int level, int* &pnts ) const{
        if ( pnts != 0 ){ delete[] pnts; }
        pnts = new int[getNumPoints( level )];
        tcopy( getNumPoints( level ), &( level_points[levels[level]] ), pnts );
}

const char* RuleGaussLaguerre::getDescription() const{
        return "Gauss-Laguerre points and weights and Lagrange Polynomials";
}

double RuleGaussLaguerre::getX( int point ) const{ return nodes[point]; }

double RuleGaussLaguerre::getWeight( int level, int point ) const{
...
double RuleGaussLaguerre::eval( int level, int point, double x ) const{
...
void RuleGaussLaguerre::buildOneLevel( int level, double* &w, double* &x ){
