...
#ifndef __TASMANIAN_SPARSE_GRID_BASE_1D_HIERARCHICAL_RULE_HPP
#define __TASMANIAN_SPARSE_GRID_BASE_1D_HIERARCHICAL_RULE_HPP
...
class OneDHierarchicalRule{
public:
        OneDHierarchicalRule();
        ~OneDHierarchicalRule();

        virtual int getMaxOrder() const;
        virtual void setMaxOrder( int max_order );
        ...
        virtual int getLevel( int point ) const; // returns the hierarchical level of a point
        virtual void getChildren( int point, int &first, int &second ) const;
        virtual void getParents( int point,  int &first, int &second ) const;

        virtual double getSupport( int level ) const; // this is actually half of the support (i.e. only one sided)
};
