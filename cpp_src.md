...
1.  [albany](albany/albany.html), examples of the usage of the ALBANY
    ...
    Ice eXperiments).
2.  [allocatable\_array](allocatable_array/allocatable_array.html), a
...
3.  [alpert\_rule](alpert_rule/alpert_rule.html), a library which can
    set up an Alpert quadrature rule for functions which are regular,
    log(x) singular, or 1/sqrt(x) singular.
4.  [anagram](anagram/anagram.html), a program which determines anagrams
    of a string, by James Cherry;
5.  [analemma](analemma/analemma.html), a program which evaluates the
...
15. [asa053](asa053/asa053.html), a library which produces sample
...
30. [asa159](asa159/asa159.html), a library which computes a random
    table with given row and column sums, by Michael Patefield. This is
    a version of Applied Statistics Algorithm 159;
31. [asa172](asa172/asa172.html), a library which generates all
...
48. [bellman\_ford](bellman_ford/bellman_ford.html), a library which
...
49. [bernstein\_polynomial](bernstein_polynomial/bernstein_polynomial.html),
    a library which evaluates the Bernstein polynomials;
50. [besselj](besselj/besselj.html), a library which evaluates Bessel J
    functions of noninteger order;
51. [beta\_nc](beta_nc/beta_nc.html), a library which evaluates the
    cumulative density function (CDF) of the noncentral Beta
    distribution;
52. [bicycle\_lock](bicycle_lock/bicycle_lock.html), a library which
    simulates the process of determining the secret combination of a
    bicycle lock, an integer between 000 and 999.
53. [bins](bins/bins.html), a library which handles geometric data
    partially sorted into bins;
54. [bisection\_integer](bisection_integer/bisection_integer.html), a
    ...
    \[A,B\].
55. [bisection\_rc](bisection_rc/bisection_rc.html), a library which
...
60. [blas1](blas1/blas1.html), a library which contains the Basic Linear
    ...
    Krogh.
61. [blas1\_c](blas1_c/blas1_c.html), a library which constitutes the
...
62. [blas1\_d](blas1_d/blas1_d.html), a library which constitutes the
    ...
    Kincaid, Fred Krogh.
63. [blas1\_s](blas1_s/blas1_s.html), a library which constitutes the
    Level 1 Basic Linear Algebra Subprograms (BLAS), using single
    precision real arithmetic, by Charles Lawson, Richard Hanson, David
    Kincaid, Fred Krogh.
64. [blas1\_z](blas1_z/blas1_z.html), a library which constitutes the
    Level 1 Basic Linear Algebra Subprograms (BLAS), using double
    precision complex arithmetic, by Charles Lawson, Richard Hanson,
    David Kincaid, Fred Krogh.
65. [blas2\_d](blas2_d/blas2_d.html), a library which constitutes the
    Level 2 Basic Linear Algebra Subprograms (BLAS), for matrix-vector
    operations, using double precision real arithmetic.
66. [blas2\_s](blas2_s/blas2_s.html), a library which constitutes the
    Level 2 Basic Linear Algebra Subprograms (BLAS), for matrix-vector
    operations, using single precision real arithmetic.
67. [blas3\_d](blas3_d/blas3_d.html), a library which constitutes the
    Level 3 Basic Linear Algebra Subprograms (BLAS) for matrix-matrix
    operations, using double precision arithmetic;
68. [blas3\_s](blas3_s/blas3_s.html), a library which constitutes the
    Level 3 Basic Linear Algebra Subprograms (BLAS) for matrix-matrix
    operations, using single precision arithmetic;
69. [blend](blend/blend.html), a library which uses transfinite
    interpolation to fill in data within a square or a cube based on
    boundary values;
70. [bmp\_io](bmp_io/bmp_io.html), a library which reads and writes
    graphics information stored in a Microsoft BMP file;
71. [bmp\_to\_ppma](bmp_to_ppma/bmp_to_ppma.html), a program which
    converts a Microsoft BMP file to an ASCII Portable Pixel Map (PPM)
    file;
72. [bmp\_to\_ppmb](bmp_to_ppmb/bmp_to_ppmb.html), a program which
    converts a Microsoft BMP file to a binary Portable Pixel Map (PPM)
    file;
73. [boost](boost/boost.html), programs which illustrate the use of some
    of the Boost libraries;
74. [box\_behnken](box_behnken/box_behnken.html), a library which
    computes a Box-Behnken design, that is, a set of arguments to sample
    the behavior of a function of multiple parameters;
75. [brent](brent/brent.html), a library which contains routines for
...
76. [brownian\_motion\_simulation](brownian_motion_simulation/brownian_motion_simulation.html),
    a library which simulates Brownian motion in an M-dimensional
    region, creating graphics files for processing by gnuplot.
77. [burgers\_solution](burgers_solution/burgers_solution.html), a
    library which evaluates exact solutions of the time-dependent 1D
    viscous Burgers equation.
78. [bvec](bvec/bvec.html), a library which demonstrates how signed
    integers can be stored as binary vectors, and arithmetic can be
    performed on them.
79. [c++\_calls\_c](c++_calls_c/c++_calls_c.html), programs which
    illustrate a C++ program calling a C function.
80. [c++\_calls\_f77](c++_calls_f77/c++_calls_f77.html), programs which
    illustrate a C++ program calling a FORTRAN77 subroutine.
81. [c++\_calls\_f90](c++_calls_f90/c++_calls_f90.html), programs which
    illustrate a C++ program calling a FORTRAN90 subroutine.
82. [c++\_condor](c++_condor/c++_condor.html), programs which illustrate
    how a C++ program can be run in batch mode using the condor queueing
    system.
83. [c4\_complex\_lib](c4_complex_lib/c4_complex_lib.html), a C++ class
    for float complex (C4) numbers;
84. [c4lib](c4lib/c4lib.html), a library which implements a basic level
    of single precision complex (C4) arithmetic.
85. [c8\_complex\_lib](c8_complex_lib/c8_complex_lib.html), a C++ class
    for double precision complex (C8) numbers;
86. [c8lib](c8lib/c8lib.html), a library which implements certain
    elementary functions for double precision complex (C8) variables;
87. [caesar](caesar/caesar.html), a library which can apply a Caesar
    Shift Cipher to a string of text.
88. [calendar\_rd](calendar_rd/calendar_rd.html), a program which
    computes the representation of a given date in a number of
    calendrical systems, by Edward Reingold and Nachum Dershowitz.
89. [calpak](calpak/calpak.html), a library which makes various calendar
    calculations;
90. [catalog](catalog/catalog.html), a program which extracts routine
    definition lines from a marked-up C, C++, FORTRAN77 or FORTRAN90
    file;
91. [cauchy\_principal\_value](cauchy_principal_value/cauchy_principal_value.html),
    a library which uses Gauss-Legendre quadrature to estimate the
    Cauchy Principal Value (CPV) of certain singular integrals.
92. [caustic\_opengl](caustic_opengl/caustic_opengl.html), a program
    which computes a caustic curve and displays it using OpenGL.
93. [cc\_io](cc_io/cc_io.html) a library which reads and writes sparse
    linear systems stored in the Compressed Column (CC) format.
94. [cc\_to\_st](cc_to_st/cc_to_st.html) a library which converts a
    sparse matrix from compressed column (CC) to sparse triplet (ST)
    format.
95. [ccn\_rule](ccn_rule/ccn_rule.html), a program which defines a
    nested Clenshaw Curtis quadrature rule.
96. [ccvt\_box](ccvt_box/ccvt_box.html), a program which carries out a
    Centroidal Voronoi Tessellation (CVT) calculation constrained to a
    box region, with points projected to the boundary;
97. [cdflib](cdflib/cdflib.html), a library which evaluates the
...
98. [cell](cell/cell.html), a library which defines a cell array, a
    ...
    triangular matrix.
99. [cellular\_automaton](cellular_automaton/cellular_automaton.html), a
    program which demonstrates the 1D Cellular Automaton known as rule
    \#30.
100. [cg](cg/cg.html), a library which implements a simple version of
...
106. [chebyshev\_series](chebyshev_series/chebyshev_series.html), a
...
107. [chebyshev1\_rule](chebyshev1_rule/chebyshev1_rule.html), a program
    which computes and writes out a Gauss-Chebyshev type 1 quadrature
    rule of given order.
108. [chebyshev2\_rule](chebyshev2_rule/chebyshev2_rule.html), a program
    which computes and writes out a Gauss-Chebyshev type 2 quadrature
    rule of given order.
109. [chrpak](chrpak/chrpak.html), a library which can manipulate
    characters and strings;
110. [circle\_arc\_grid](circle_arc_grid/circle_arc_grid.html), a
    program which computes grid points along a circular arc.
111. [circle\_integrals](circle_integrals/circle_integrals.html), a
    library which returns the exact value of the integral of any
    monomial over the circumference of the unit circle in 2D.
112. [circle\_monte\_carlo](circle_monte_carlo/circle_monte_carlo.html),
    ...
    2D;
113. [circle\_rule](circle_rule/circle_rule.html), a library which
    computes quadrature rules for the unit circle in 2D, that is, the
    circumference of the circle of radius 1 and center (0,0).
114. [circle\_segment](circle_segment/circle_segment.html), a library
...
115. [cities](cities/cities.html), a library which works with
    city-to-city distance matrices;
116. [clapack](clapack/clapack.html), programs which illustrate the use
    ...
    complex arithmetic.
117. [clapack\_sc](clapack_sc/clapack_sc.html), programs which
...
118. [clausen](clausen/clausen.html), a library which evaluates a
    Chebyshev interpolant to the Clausen function Cl2(x).
119. [clenshaw\_curtis\_rule](clenshaw_curtis_rule/clenshaw_curtis_rule.html),
    a program which implements a Clenshaw Curtis quadrature rule;
120. [cnf\_io](cnf_io/cnf_io.html), a library which reads or writes
    Conjunctive Normal Form (CNF) data files, used to store boolean
    formulas.
121. [collatz\_recursive](collatz_recursive/collatz_recursive.html), a
    library which demonstrates recursive programming by considering the
    simple Collatz 3n+1 problem.
122. [colored\_noise](colored_noise/colored_noise.html), a library which
    generates samples of noise obeying a 1/f\^alpha power law.
123. [combination\_lock](combination_lock/combination_lock.html), a
    program which simulates the process of determining the secret
    combination of a lock.
124. [combo](combo/combo.html), a library which includes routines for
...
125. [communicator\_mpi](communicator_mpi/communicator_mpi.html), a
    program which creates new communicators involving a subset of
    initial set of MPI processes in the default communicator
    MPI\_COMM\_WORLD.
126. [compass\_search](compass_search/compass_search.html), a library
    ...
    derivatives.
127. [complex\_numbers](complex_numbers/complex_numbers.html), a program
    which demonstrates some simple features involved in the use of
    complex numbers in C programming.
128. [condition](condition/condition.html), a library which implements
    methods of computing or estimating the condition number of a matrix.
129. [continued\_fraction](continued_fraction/continued_fraction.html) a
    library which implements some simple algorithms for dealing with
    simple and generalized continued fractions.
130. [cordic](cordic/cordic.html), a library which computes a few
    special functions using the CORDIC algorithm.
131. [correlation](correlation/correlation.html), a library which
    contains examples of statistical correlation functions.
132. [cosine\_transform](cosine_transform/cosine_transform.html), a
    library which demonstrates some simple properties of the discrete
    cosine transform (DCT).
133. [cpp](cpp/cpp.html), programs which illustrate some features of the
    C++ language;
134. [cpp\_arrays](cpp_arrays/cpp_arrays.html), programs which
    illustrate the use of vectors, matrices and tensors.
135. [cpp\_files](cpp_files/cpp_files.html), programs which illustrate
    the use of files.
136. [cpp\_intrinsics](cpp_intrinsics/cpp_intrinsics.html), a program
    which illustrates the use of intrinsic functions supplied by C++.
137. [cpp\_random](cpp_random/cpp_random.html), programs which
    illustrate the use of the C++ random number generator (RNG)
    functions.
138. [cr2crlf](cr2crlf/cr2crlf.html), a program which converts carriage
    returns to carriage returns + linefeeds in a file;
139. [cr2lf](cr2lf/cr2lf.html), a program which converts carriage
    returns to linefeeds in a file;
140. [crc](crc/crc.html), a library which computes the cyclic redundancy
    checksum (CRC) as used by the PNG data format.
141. [crrm](crrm/crrm.html), a program which removes carriage returns
    from a file;
142. [cube\_arbq\_rule](cube_arbq_rule/cube_arbq_rule.html), a library
    ...
    Zydrunas Gimbutas.
143. [cube\_exactness](cube_exactness/cube_exactness.html), a library
    which investigates the polynomial exactness of quadrature rules over
    the interior of a cube in 3D.
144. [cube\_felippa\_rule](cube_felippa_rule/cube_felippa_rule.html), a
    library which returns Felippa's quadratures rules for approximating
    integrals over the interior of a cube in 3D.
145. [cube\_grid](cube_grid/cube_grid.html), a library which computes a
    grid of points over the interior of a cube in 3D.
146. [cube\_integrals](cube_integrals/cube_integrals.html), a library
    which returns the exact value of the integral of any monomial over
    the interior of the unit cube in 3D.
147. [cube\_monte\_carlo](cube_monte_carlo/cube_monte_carlo.html), a
...
148. [cvt](cvt/cvt.html), a library which computes an N point CVT
    Centroidal Voronoi Tessellation (CVT) in M dimensions;
149. [cvt\_dataset](cvt_dataset/cvt_dataset.html), a program which
    creates a Centroidal Voronoi Tessellation (CVT) dataset;
150. [cycle\_brent](cycle_brent/cycle_brent.html), a library which
...
151. [cycle\_floyd](cycle_floyd/cycle_floyd.html), a library which
    ...
    Floyd's method.
152. [cyclic\_reduction](cyclic_reduction/cyclic_reduction.html), a
    library which solves a tridiagonal linear system using cyclic
    reduction;
153. [decomment](decomment/decomment.html), a program which removes from
    a file every line that begins with "\#" (a common convention for
    indicating comments);
154. [delaunay\_tree\_2d](delaunay_tree_2d/delaunay_tree_2d.html), a
    program which computes the Delaunay triangulation of a 2D dataset,
    by Olivier Devillers.
155. [diaphony](diaphony/diaphony.html), a program which reads a file of
    N points in M dimensions and computes its diaphony, a measure of
    point dispersion.
156. [differ](differ/differ.html), determines the finite difference
...
157. [dijkstra](dijkstra/dijkstra.html), a library which implements a
    simple version of Dijkstra's minimum distance algorithm for graphs.
158. [dijkstra\_openmp](dijkstra_openmp/dijkstra_openmp.html), a program
    which uses OpenMP to parallelize a simple example of Dijkstra's
    minimum distance algorithm for graphs.
159. [disk\_grid](disk_grid/disk_grid.html), a library which computes
...
160. [disk\_integrals](disk_integrals/disk_integrals.html), a library
    which returns the exact value of the integral of any monomial over
    the interior of the unit disk in 2D.
161. [disk\_monte\_carlo](disk_monte_carlo/disk_monte_carlo.html), a
    library which applies a Monte Carlo method to estimate integrals of
    a function over the interior of the unit disk in 2D;
162. [disk\_quarter\_monte\_carlo](disk_quarter_monte_carlo/disk_quarter_monte_carlo.html),
...
171. [ellipse\_grid](ellipse_grid/ellipse_grid.html), a library which
    computes grid points over the interior of an ellipse in 2D.
172. [ellipse\_monte\_carlo](ellipse_monte_carlo/ellipse_monte_carlo.html),
    a library which uses the Monte Carlo method to estimate the value of
    integrals over the interior of an ellipse in 2D.
173. [ellipsoid\_grid](ellipsoid_grid/ellipsoid_grid.html), a library
    which computes a grid of points over the interior of an ellipsoid in
    3D.
174. [ellipsoid\_monte\_carlo](ellipsoid_monte_carlo/ellipsoid_monte_carlo.html),
    a library which uses the Monte Carlo method to estimate the value of
    integrals over the interior of an ellipsoid in M dimensions.
175. [exactness](exactness/exactness.html), a library which investigates
...
176. [fair\_dice\_simulation](fair_dice_simulation/fair_dice_simulation.html),
    a program which simulates N tosses of 2 dice, creating graphics
    files for processing by gnuplot.
177. [fastgl](fastgl/fastgl.html), a library which carries out the rapid
    computation of the Kth value and weight of an N point Gauss-Legendre
    quadrature rule, by Ignace Bogaert.
178. [faure](faure/faure.html), a library which computes elements of the
    Faure M-dimensional Quasi Mont Carlo (QMC) sequence;
179. [faure\_dataset](faure_dataset/faure_dataset.html), a program which
    creates a Faure Quasi Mont Carlo (QMC) dataset;
180. [fd\_predator\_prey](fd_predator_prey/fd_predator_prey.html), a
    program which solves a pair of predator prey ordinary differential
    equations (ODE's) using the finite difference method (FDM).
181. [fd1d\_advection\_diffusion\_steady](fd1d_advection_diffusion_steady/fd1d_advection_diffusion_steady.html),
...
182. [fd1d\_advection\_ftcs](fd1d_advection_ftcs/fd1d_advection_ftcs.html),
    a program which applies the finite difference method (FDM) to solve
    ...
    gnuplot.
183. [fd1d\_advection\_lax](fd1d_advection_lax/fd1d_advection_lax.html),
    a program which applies the finite difference method (FDM) to solve
    the time-dependent advection equation ut = - c \* ux in one spatial
...
184. [fd1d\_advection\_lax\_wendroff](fd1d_advection_lax_wendroff/fd1d_advection_lax_wendroff.html),
    a program which applies the finite difference method (FDM) to solve
    the time-dependent advection equation ut = - c \* ux in one spatial
    ...
    processing by gnuplot.
185. [fd1d\_burgers\_lax](fd1d_burgers_lax/fd1d_burgers_lax.html), a
...
186. [fd1d\_burgers\_leap](fd1d_burgers_leap/fd1d_burgers_leap.html), a
...
194. [fem\_io](fem_io/fem_io.html), a library which reads or writes FEM
    files, which describe the nodes, elements, and function values
    associated with a finite element method (FEM) model;
195. [fem\_to\_gmsh](fem_to_gmsh/fem_to_gmsh.html), a program which
...
196. [fem\_to\_medit](fem_to_medit/fem_to_medit.html), a program which
    reads a pair of FEM files defining node coordinates and elements, of
    ...
    file.
197. [fem\_to\_mesh](fem_to_mesh/fem_to_mesh.html), a program which
    ...
    as a MESH file.
198. [fem\_to\_triangle](fem_to_triangle/fem_to_triangle.html), a
...
199. [fem\_to\_xml](fem_to_xml/fem_to_xml.html), reads a pair of FEM
...
200. [fem1d](fem1d/fem1d.html), a program which applies the finite
    element method (FEM), using piecewise linear basis functions, to a
    linear two point boundary value problem (BVP) in 1D;
201. [fem1d\_adaptive](fem1d_adaptive/fem1d_adaptive.html), a program
...
202. [fem1d\_bvp\_linear](fem1d_bvp_linear/fem1d_bvp_linear.html), a
...
203. [fem1d\_bvp\_quadratic](fem1d_bvp_quadratic/fem1d_bvp_quadratic.html),
    ...
    (BVP) in one spatial dimension, and compares the computed and exact
    solutions with the L2 and seminorm errors.
204. [fem1d\_heat\_steady](fem1d_heat_steady/fem1d_heat_steady.html), a
    program which implements the finite element method (FEM) for the
    steady (time independent) 1D heat equation;
205. [fem1d\_lagrange](fem1d_lagrange/fem1d_lagrange.html), a library
...
206. [fem1d\_nonlinear](fem1d_nonlinear/fem1d_nonlinear.html), a program
    ...
    (BVP) in 1D;
207. [fem1d\_pack](fem1d_pack/fem1d_pack.html), a library which contains
    utilities for 1D finite element method (FEM) calculations.
208. [fem1d\_pmethod](fem1d_pmethod/fem1d_pmethod.html), a program which
    applies the p-method version of the finite element method (FEM) to a
    linear two point boundary value problem (BVP) in 1D;
209. [fem1d\_project](fem1d_project/fem1d_project.html), a program which
...
210. [fem1d\_sample](fem1d_sample/fem1d_sample.html), a program which
    ...
    points.
211. [fem2d\_bvp\_linear](fem2d_bvp_linear/fem2d_bvp_linear.html), a
    program which applies the finite element method (FEM), with
    ...
    and seminorm errors.
212. [fem2d\_bvp\_quadratic](fem2d_bvp_quadratic/fem2d_bvp_quadratic.html),
    a program which applies the finite element method (FEM), with
...
217. [fem2d\_mesh\_display\_opengl](fem2d_mesh_display_opengl/fem2d_mesh_display_opengl.html),
...
218. [fem2d\_pack](fem2d_pack/fem2d_pack.html), a library which
    implements various 2D finite element method (FEM) calculations;
219. [fem2d\_poisson](fem2d_poisson/fem2d_poisson.html), a program which
    applies the finite element method (FEM) to solve Poisson's equation
    in an arbitrary triangulated region in 2D;
220. [fem2d\_poisson\_cg](fem2d_poisson_cg/fem2d_poisson_cg.html), a
...
221. [fem2d\_poisson\_cg\_baffle](fem2d_poisson_cg_baffle/fem2d_poisson_cg_baffle.html),
...
222. [fem2d\_poisson\_cg\_ell](fem2d_poisson_cg_ell/fem2d_poisson_cg_ell.html),
...
223. [fem2d\_poisson\_cg\_lake](fem2d_poisson_cg_lake/fem2d_poisson_cg_lake.html),
...
224. [fem2d\_poisson\_ell](fem2d_poisson_ell/fem2d_poisson_ell.html), a
...
225. [fem2d\_poisson\_lake](fem2d_poisson_lake/fem2d_poisson_lake.html),
    a library which defines the geometry of a lake-shaped region, as
    well as boundary conditions for a given Poisson problem, and is
    called by fem2d\_poisson as part of a solution procedure.
226. [fem2d\_poisson\_rectangle](fem2d_poisson_rectangle/fem2d_poisson_rectangle.html),
    ...
    elements.
227. [fem2d\_poisson\_rectangle\_linear](fem2d_poisson_rectangle_linear/fem2d_poisson_rectangle_linear.html),
    a program which solves the 2D Poisson equation on a rectangle, using
    the finite element method (FEM), and piecewise linear triangular
    elements.
228. [fem2d\_poisson\_sparse](fem2d_poisson_sparse/fem2d_poisson_sparse.html),
...
229. [fem2d\_poisson\_sparse\_baffle](fem2d_poisson_sparse_baffle/fem2d_poisson_sparse_baffle.html),
    ...
    solution procedure.
230. [fem2d\_poisson\_sparse\_ell](fem2d_poisson_sparse_ell/fem2d_poisson_sparse_ell.html),
    a library which defines the geometry of an L-shaped region, as well
    as boundary conditions for a given Poisson problem, and is called by
    fem2d\_poisson\_sparse as part of a solution procedure.
231. [fem2d\_poisson\_sparse\_lake](fem2d_poisson_sparse_lake/fem2d_poisson_sparse_lake.html),
    a library which defines the geometry of a lake-shaped region, as
    well as boundary conditions for a given Poisson problem, and is
    called by fem2d\_poisson\_sparse as part of a solution procedure.
232. [fem2d\_project](fem2d_project/fem2d_project.html), a program which
    projects a function f(x,y), given as data, into a given finite
    element space of piecewise linear triangular elements.
233. [fem2d\_project\_function](fem2d_project_function/fem2d_project_function.html),
    ...
    elements.
234. [fem2d\_sample](fem2d_sample/fem2d_sample.html), a program which
    ...
    arbitrary points.
235. [fem3d\_pack](fem3d_pack/fem3d_pack.html), a library which contains
    utilities for 3D finite element method (FEM) calculations.
236. [fem3d\_project](fem3d_project/fem3d_project.html), a program which
    ...
    method (FEM).
237. [fem3d\_sample](fem3d_sample/fem3d_sample.html), a program library
    which evaluates a finite element function defined on a 3D
    tetrahedral mesh.
238. [fern\_opengl](fern_opengl/fern_opengl.html), a program which uses
    OpenGL to display the Barnsley fractal fern.
239. [feynman\_kac\_1d](feynman_kac_1d/feynman_kac_1d.html), a program
    ...
    the boundary.
240. [feynman\_kac\_2d](feynman_kac_2d/feynman_kac_2d.html), a program
    which demonstrates the use of the Feynman-Kac algorithm to solve
    Poisson's equation in a 2D ellipse by averaging stochastic paths to
    the boundary.
241. [feynman\_kac\_3d](feynman_kac_3d/feynman_kac_3d.html), a program
    which demonstrates the use of the Feynman-Kac algorithm to solve
    Poisson's equation in a 3D ellipsoid by averaging stochastic paths
    to the boundary.
242. [fft\_openmp](fft_openmp/fft_openmp.html), a program which
    demonstrates the computation of a Fast Fourier Transform (FFT) in
    parallel, using OpenMP.
243. [fft\_serial](fft_serial/fft_serial.html), a program which computes
    a Fast Fourier Transform (FFT), and is intended as a starting point
    for implementing an OpenMP parallel version.
244. [file\_name\_sequence](file_name_sequence/file_name_sequence.html),
...
245. [files\_multiple](files_multiple/files_multiple.html), a program
...
246. [filon](filon/filon.html), a library which can approximate
    integrals in which the integrand includes an oscillatory factor of
    sin(k\*x) or cos(k\*x).
247. [filum](filum/filum.html), a library which performs various
    operations with files;
248. [fire\_serial](fire_serial/fire_serial.html), a program which
...
249. [floyd](floyd/floyd.html), a library which implements Floyd's
    algorithm for finding the shortest distance between pairs of nodes
    on a directed graph.
250. [fn](fn/fn.html), a library which evaluates elementary and special
...
261. [gmsh\_io](gmsh_io/gmsh_io.html), a library which can read or write
    some of the files created by the Gmsh program for 1D, 2D or 3D
    meshes used by the finite element method (FEM).
262. [gmsh\_to\_fem](gmsh_to_fem/gmsh_to_fem.html), a program which
...
263. [gnuplot](gnuplot/gnuplot.html), programs which illustrate how a
    program can write data and command files so that gnuplot can create
    plots of the program results.
264. [gnuplot\_i\_examples](gnuplot_i_examples/gnuplot_i_examples.html),
    programs which demonstrate the use of the gnuplot\_i library for
    interactive runtime gnuplot graphics.
265. [gprof](gprof/gprof.html), programs which illustrate the use of the
    gprof program performance monitor;
266. [graphics\_examples](graphics_examples/graphics_examples.html),
    programs which illustrate how various kinds of data can be displayed
    and analyzed graphically.
267. [grf\_display\_opengl](grf_display_opengl/grf_display_opengl.html),
    a program which reads a GRF file defining a mathematical graph and
    displays it in the OpenGL graphics window.
268. [grf\_io](grf_io/grf_io.html), a library which reads or writes a
    GRF file;
269. [grid](grid/grid.html), a library which randomly selects N distinct
    points from a uniform grid in the unit hypercube in M-dimensional
    space.
270. [grid\_to\_bmp](grid_to_bmp/grid_to_bmp.html), a program which
    reads a text file of data on a rectangular grid and creates a BMP
    file containing a color image of the data.
271. [gsl](gsl/gsl.html), programs which illustrate the use of the Gnu
    Scientific Library (GSL);
272. [haar](haar/haar.html), a library which computes the Haar transform
    of data.
273. [halton](halton/halton.html), a library which computes elements of
    a Halton Quasi Monte Carlo (QMC) sequence, using a simple interface.
274. [halton\_advanced](halton_advanced/halton_advanced.html), a library
...
275. [halton\_dataset](halton_dataset/halton_dataset.html), a program
    which creates a Halton Quasi Mont Carlo (QMC) sequence;
276. [hammersley](hammersley/hammersley.html), a library which computes
    elements of a Hammersley Quasi Monte Carlo (QMC) sequence, using a
    simple interface.
277. [hammersley\_advanced](hammersley_advanced/hammersley_advanced.html),
...
278. [hammersley\_dataset](hammersley_dataset/hammersley_dataset.html),
    a program which creates a Hammersley Quasi Mont Carlo (QMC)
    sequence;
279. [hankel\_cholesky](hankel_cholesky/hankel_cholesky.html), a library
    which computes the upper Cholesky factor R of a nonnegative definite
    symmetric H matrix so that H = R' \* R.
280. [hankel\_pds](hankel_pds/hankel_pds.html), a library which can
    ...
    L'.
281. [hb\_io](hb_io/hb_io.html), a library which reads and writes
    matrices in the Harwell Boeing (HB) sparse matrix format;
282. [hb\_io\_new](hb_io_new/hb_io_new.html), a library which reads and
    writes matrices in the Harwell Boeing (HB) sparse matrix format,
    modified and improved by Reinhard Resch.
283. [hdf5](hdf5/hdf5.html), functions which demonstrate the use of the
    HDF5 library and file format.
284. [heat\_mpi](heat_mpi/heat_mpi.html), a program which demonstrates
    the use of the Message Passing Interface (MPI), by solving the 1D
    time dependent heat equation.
285. [heated\_plate](heated_plate/heated_plate.html), a program which
    ...
    parallel version.
286. [heated\_plate\_openmp](heated_plate_openmp/heated_plate_openmp.html),
    a program which solves the steady (time independent) heat equation
    in a 2D rectangular region, using OpenMP to run in parallel.
287. [hello](hello/hello.html), a program which prints out "Hello,
    world!".
288. [hello\_mpi](hello_mpi/hello_mpi.html), a program which prints out
    "Hello, world!" using the Message Passing Interface (MPI) for
    parallel programming.
289. [hello\_openmp](hello_openmp/hello_openmp.html), a program which
    prints out "Hello, world!" within the OpenMP parallel programming
    environment.
290. [hermite](hermite/hermite.html), a library which computes the
    Hermite interpolant, a polynomial that matches function values and
    derivatives.
291. [hermite\_cubic](hermite_cubic/hermite_cubic.html), a library which
...
292. [hermite\_exactness](hermite_exactness/hermite_exactness.html), a
...
293. [hermite\_polynomial](hermite_polynomial/hermite_polynomial.html),
    ...
    functions.
294. [hermite\_product\_polynomial](hermite_product_polynomial/hermite_product_polynomial.html),
    ...
    polynomials.
295. [hermite\_rule](hermite_rule/hermite_rule.html), a program which
...
296. [hermite\_test\_int](hermite_test_int/hermite_test_int.html), a
    library which defines test integrands for Hermite integrals with
    density exp(-x\^2) over the interval (-oo,+oo).
297. [hexdump](hexdump/hexdump.html), a program which produces a
    hexadecimal dump of a file;
298. [high\_card\_simulation](high_card_simulation/high_card_simulation.html),
...
299. [hilbert\_curve](hilbert_curve/hilbert_curve.html), a library which
    computes the sequence of discrete Hilbert curves whose limit is a
    space-filling curve.
300. [histogram\_data\_2d\_sample](histogram_data_2d_sample/histogram_data_2d_sample.html),
...
301. [htmlindex](htmlindex/htmlindex.html), a program which creates a
    skeleton HTML page for a marked-up C, C++ or FORTRAN file;
302. [hyperball\_integrals](hyperball_integrals/hyperball_integrals.html),
...
303. [hyperball\_monte\_carlo](hyperball_monte_carlo/hyperball_monte_carlo.html),
    a library which applies a Monte Carlo method to estimate the
    integral of a function over the interior of the unit ball in M
    dimensions;
304. [hyperball\_volume\_monte\_carlo](hyperball_volume_monte_carlo/hyperball_volume_monte_carlo.html),
    a program which applies a Monte Carlo procedure to estimate the
    volume of the unit ball in M dimensions;
305. [hypercube\_exactness](hypercube_exactness/hypercube_exactness.html),
...
306. [hypercube\_grid](hypercube_grid/hypercube_grid.html), a library
    which computes a grid of points over the interior of a hypercube in
    M dimensions.
307. [hypercube\_integrals](hypercube_integrals/hypercube_integrals.html),
    a library which returns the exact value of the integral of any
    monomial over the interior of the unit hypercube in M dimensions.
308. [hypercube\_monte\_carlo](hypercube_monte_carlo/hypercube_monte_carlo.html),
    a library which applies a Monte Carlo method to estimate the
    integral of a function over the interior of the unit hypercube in M
    dimensions.
309. [hypersphere\_integrals](hypersphere_integrals/hypersphere_integrals.html),
    a library which returns the exact value of the integral of any
    monomial over the surface of the unit hypersphere in M dimensions.
310. [hypersphere\_monte\_carlo](hypersphere_monte_carlo/hypersphere_monte_carlo.html),
    a library which applies a Monte Carlo method to estimate the
    integral of a function on the surface of the unit sphere in M
    dimensions;
311. [hypersphere\_properties](hypersphere_properties/hypersphere_properties.html),
...
312. [i2\_binary\_to\_ascii](i2_binary_to_ascii/i2_binary_to_ascii.html),
    a program which converts a file of short int (I2) data from binary
    to ASCII format.
313. [i4lib](i4lib/i4lib.html), a library which contains many utility
    routines, using single precision integer (I4) arithmetic.
314. [i8lib](i8lib/i8lib.html), a library which contains many utility
    routines, using double precision integer (I8) arithmetic.
315. [ice\_io](ice_io/ice_io.html), a library which can read or write an
    ICE dataset, which defines a 3D grid, using the NETCDF file format.
316. [ice\_to\_mesh](ice_to_mesh/ice_to_mesh.html), a program which
    reads a NETCDF file containing an ICE dataset and rewrites the
    information as a MESH file.
317. [ihs](ihs/ihs.html), a library which computes elements of the
    Improved Hypercube Sampling (IHS) M-dimensional Quasi Mont Carlo
    (QMC) sequence;
318. [ihs\_dataset](ihs_dataset/ihs_dataset.html), a program which
    creates an Improved Hypercube Sampling (IHS) dataset;
319. [image\_components](image_components/image_components.html), a
    library which seeks the connected nonzero or nonblack components of
    an image or integer array.
320. [image\_denoise](image_denoise/image_denoise.html), a library which
    applies simple filtering operations to a noisy image.
321. [image\_edge](image_edge/image_edge.html), a library which
    demonstrates a simple procedure for edge detection in images.
322. [index](index/index.html), a library which converts a M-dimensional
    ...
    major conventions.
323. [int\_exactness](int_exactness/int_exactness.html), a program which
    tests the polynomial exactness of a 1D quadrature rule for a finite
    interval;
324. [int\_exactness\_chebyshev1](int_exactness_chebyshev1/int_exactness_chebyshev1.html),
    a program which tests the polynomial exactness of Gauss-Chebyshev
    type 1 quadrature rules.
325. [int\_exactness\_chebyshev2](int_exactness_chebyshev2/int_exactness_chebyshev2.html),
    a program which tests the polynomial exactness of Gauss-Chebyshev
    type 2 quadrature rules.
326. [int\_exactness\_gen\_hermite](int_exactness_gen_hermite/int_exactness_gen_hermite.html),
    a program which tests the polynomial exactness of generalized
    Gauss-Hermite quadrature rules.
327. [int\_exactness\_gen\_laguerre](int_exactness_gen_laguerre/int_exactness_gen_laguerre.html),
    a program which tests the polynomial exactness of generalized
    Gauss-Laguerre quadrature rules.
328. [interp](interp/interp.html), a library which can be used for
    parameterizing and interpolating data;
329. [isbn](isbn/isbn.html), a library which can determine the check
    digit for an International Standard Book Number or report whether a
    given ISBN is valid.
330. [ising\_2d\_simulation](ising_2d_simulation/ising_2d_simulation.html),
...
331. [ivcon](ivcon/ivcon.html), a program which converts between various
    3D graphic file formats;
332. [jacobi](jacobi/jacobi.html), a program which implements the Jacobi
    iteration for linear systems.
333. [jacobi\_eigenvalue](jacobi_eigenvalue/jacobi_eigenvalue.html), a
...
343. [lagrange\_interp\_1d](lagrange_interp_1d/lagrange_interp_1d.html),
    a library which defines and evaluates the Lagrange polynomial p(x)
    which interpolates a set of data depending on a 1D argument, so that
    p(x(i)) = y(i).
344. [lagrange\_interp\_2d](lagrange_interp_2d/lagrange_interp_2d.html),
...
347. [laguerre\_exactness](laguerre_exactness/laguerre_exactness.html),
    ...
    \[0,+oo).
348. [laguerre\_polynomial](laguerre_polynomial/laguerre_polynomial.html),
    a library which evaluates the Laguerre polynomial, the generalized
    Laguerre polynomials, and the Laguerre function.
349. [laguerre\_rule](laguerre_rule/laguerre_rule.html), a program which
    ...
    the interval \[0,+oo).
350. [laguerre\_test\_int](laguerre_test_int/laguerre_test_int.html), a
    library which implements test problems for for Gauss-Laguerre type
    quadrature rules for approximating the integral of a function with
    density exp(-x) over the interval \[0,+oo).
351. [latin\_center](latin_center/latin_center.html), a library which
    computes Latin Center Squares of N points in M dimensions;
352. [latin\_center\_dataset](latin_center_dataset/latin_center_dataset.html),
    a program which creates a Latin Center Hypercube dataset;
353. [latin\_cover](latin_cover/latin_cover.html), a library which
    produces N Latin squares which cover an NxN square, or NxN Latin
    cubes which cover an NxNxN cube.
354. [latin\_edge](latin_edge/latin_edge.html), a library which computes
    Latin Edge Squares of N points in M dimensions;
355. [latin\_edge\_dataset](latin_edge_dataset/latin_edge_dataset.html),
    a program which creates a Latin Edge Hypercube dataset;
356. [latin\_random](latin_random/latin_random.html), a library which
    computes Latin Random Squares of N points in M dimensions;
357. [latin\_random\_dataset](latin_random_dataset/latin_random_dataset.html),
    a program which creates a Latin Random Hypercube dataset;
358. [latinize](latinize/latinize.html), a library which adjusts N
    points in M dimensions to form a Latin Hypercube;
359. [lattice\_rule](lattice_rule/lattice_rule.html), a library which
    returns lattice rules for M-dimensional integration;
360. [lcvt](lcvt/lcvt.html), a library which creates a Latinized
    Centroidal Voronoi Tessellation (CVT);
361. [lcvt\_dataset](lcvt_dataset/lcvt_dataset.html), a program which
    creates a Latinized Centroidal Voronoi Tessellation (CVT);
362. [lebesgue](lebesgue/lebesgue.html), a library which is given a set
...
363. [legendre\_exactness](legendre_exactness/legendre_exactness.html),
    ...
    the interval \[-1,+1\].
364. [legendre\_polynomial](legendre_polynomial/legendre_polynomial.html),
    a library which evaluates the Legendre polynomial and associated
    functions.
365. [legendre\_product\_polynomial](legendre_product_polynomial/legendre_product_polynomial.html),
    ...
    polynomials.
366. [legendre\_rule](legendre_rule/legendre_rule.html), a program which
    writes out a Gauss-Legendre quadrature rule of given order.
367. [legendre\_rule\_fast](legendre_rule_fast/legendre_rule_fast.html),
    a program which uses a fast (order N) algorithm to compute a
    Gauss-Legendre quadrature rule of given order.
368. [legendre\_shifted\_polynomial](legendre_shifted_polynomial/legendre_shifted_polynomial.html),
    a library which evaluates the shifted Legendre polynomial, with the
    domain \[0,1\].
369. [lf2cr](lf2cr/lf2cr.html), a program which converts linefeeds to
    carriage returns in a file;
370. [lf2crlf](lf2crlf/lf2crlf.html), a program which converts linefeeds
    to carriage return + linefeeds in a file;
371. [lfrm](lfrm/lfrm.html), a program which removes all linefeed
    characters from a file;
372. [life\_opengl](life_opengl/life_opengl.html), a program which uses
    OpenGL to display the evolution of John Conway's Game of Life.
373. [life\_serial](life_serial/life_serial.html), a program which
    computes a few steps of the evolution of John Conway's Game of Life,
    intended as a starting point for implementing a parallel version.
374. [lights\_out\_opengl](lights_out_opengl/lights_out_opengl.html), a
    program which sets up a Lights Out game and allows the user to solve
    it, using the OpenGL graphics window.
375. [line\_cvt\_lloyd](line_cvt_lloyd/line_cvt_lloyd.html), a library
...
376. [line\_fekete\_rule](line_fekete_rule/line_fekete_rule.html), a
    ...
    segment in 1D.
377. [line\_felippa\_rule](line_felippa_rule/line_felippa_rule.html), a
    library which returns Felippa's quadratures rules for approximating
    integrals over the interior of a line segment in 1D.
378. [line\_grid](line_grid/line_grid.html), a library which computes a
    grid of points over the interior of a line segment in 1D.
379. [line\_integrals](line_integrals/line_integrals.html), a library
    which returns the exact value of the integral of any monomial over
    the length of the unit line in 1D.
380. [line\_monte\_carlo](line_monte_carlo/line_monte_carlo.html), a
    library which applies a Monte Carlo method to estimate the integral
    of a function over the length of the unit line in 1D.
381. [line\_ncc\_rule](line_ncc_rule/line_ncc_rule.html), a library
...
382. [line\_nco\_rule](line_nco_rule/line_nco_rule.html), a library
    which computes a Newton Cotes Open (NCO) quadrature rule, using
    equally spaced points, over the interior of a line segment in 1D.
383. [linpack](linpack/linpack.html), a library which solves linear
...
384. [linpack\_bench](linpack_bench/linpack_bench.html), a program which
    is a version of the LINPACK benchmark program;
385. [linpack\_c](linpack_c/linpack_c.html), a library which factors
...
386. [linpack\_d](linpack_d/linpack_d.html), a library which factors
    matrices in a variety of formats, and solving the associated linear
    systems, using double precision real arithmetic; it includes a
    function for the singular value decomposition (SVD) of a rectangular
    matrix, by Jack Dongarra, Cleve Moler, Jim Bunch, Pete Stewart.
387. [linpack\_s](linpack_s/linpack_s.html), a library which factors
    matrices in a variety of formats, and solving the associated linear
    systems, using single precision real arithmetic; it includes a
    function for the singular value decomposition (SVD) of a rectangular
    matrix, by Jack Dongarra, Cleve Moler, Jim Bunch, Pete Stewart.
388. [linpack\_z](linpack_z/linpack_z.html), a library which factors
    matrices in a variety of formats, and solving the associated linear
    systems, using double precision complex arithmetic; it includes a
    function for the singular value decomposition (SVD) of a rectangular
    matrix, by Jack Dongarra, Cleve Moler, Jim Bunch, Pete Stewart.
389. [lissajous\_opengl](lissajous_opengl/lissajous_opengl.html), a
    program which computes a Lissajous curve and displays it using
    OpenGL.
390. [llsq](llsq/llsq.html), a library which solves the simple linear
    ...
    N data points.
391. [lobatto\_polynomial](lobatto_polynomial/lobatto_polynomial.html),
    a library which evaluates Lobatto polynomials similar to Legendre
    polynomials except that they are 0 at both endpoints.
392. [local\_min\_rc](local_min_rc/local_min_rc.html), a library which
...
393. [log\_normal](log_normal/log_normal.html), a library which returns
    quantities related to the log normal Probability Distribution
    Function (PDF).
394. [log\_normal\_truncated\_ab](log_normal_truncated_ab/log_normal_truncated_ab.html),
    ...
    \[A,B\].
395. [lorenz\_ode](lorenz_ode/lorenz_ode.html), a program which
    approximates solutions to the Lorenz system, creating output files
    that can be displayed by gnuplot.
396. [luhn](luhn/luhn.html), a library which can compute the Luhn check
    digit for a string, or verify a string, as used for error detection
    in credit card numbers.
397. [machar](machar/machar.html), a library which dynamically computes
    the values of various machine constants, by William Cody;
398. [machine](machine/machine.html), a library which returns tabulated
    values of the constants associated with computer arithmetic.
399. [makefiles](makefiles/makefiles.html), programs which illustrate
    the use of makefiles for maintaining a software project;
400. [mandelbrot](mandelbrot/mandelbrot.html), a program which generates
    an ASCII Portable Pixel Map (PPM) image of the Mandelbrot set;
401. [mandelbrot\_openmp](mandelbrot_openmp/mandelbrot_openmp.html), a
...
402. [mandelbrot\_ppm](mandelbrot_ppm/mandelbrot_ppm.html), a program
    which generates a binary Portable Pixel Map (PPM) image of the
    Mandelbrot set, by Eric Weeks.
403. [matrix\_exponential](matrix_exponential/matrix_exponential.html),
    a library which demonstrates some simple approaches to the problem
    of computing the exponential of a matrix.
404. [md](md/md.html), a program which carries out a molecular dynamics
    simulation, and is intended as a starting point for implementing an
    OpenMP parallel version.
405. [md\_openmp](md_openmp/md_openmp.html), a program which carries out
    a molecular dynamics simulation in parallel using OpenMP.
406. [medit\_io](medit_io/medit_io.html), a library which can read or
...
407. [medit\_to\_fem](medit_to_fem/medit_to_fem.html), a program which
    ...
    FEM format.
408. [memory\_test](memory_test/memory_test.html), a program which tests
    the memory available on the computer by declaring and using big
    vectors.
409. [mesh\_bandwidth](mesh_bandwidth/mesh_bandwidth.html), a program
    which returns the geometric bandwidth associated with a mesh of
    elements of any order and in a space of M dimensions.
410. [mesh\_display\_opengl](mesh_display_opengl/mesh_display_opengl.html),
...
420. [monomial\_value](monomial_value/monomial_value.html), a library
    which evaluates a monomial in M dimensions.
421. [monte\_carlo\_rule](monte_carlo_rule/monte_carlo_rule.html), a
...
422. [mpas\_draw](mpas_draw/mpas_draw.html), a program which reads
...
423. [mpas\_grid\_display\_opengl](mpas_grid_display_opengl/mpas_grid_display_opengl.html),
...
424. [mpi](mpi/mpi.html), programs which illustrate the use of the MPI
    Message Passing Interface library, for parallel programming;
425. [mpi\_stubs](mpi_stubs/mpi_stubs.html), a library which may be used
    if a parallel program is to be compiled, loaded and run in a serial
    machine;
426. [multigrid\_poisson\_1d](multigrid_poisson_1d/multigrid_poisson_1d.html),
    a library which applies the multigrid method to a discretized
    version of the 1D Poisson equation.
427. [multitask\_mpi](multitask_mpi/multitask_mpi.html), a program which
    demonstrates how to multitask, that is, to execute several unrelated
    and distinct tasks simultaneously, using MPI for parallel execution.
428. [multitask\_openmp](multitask_openmp/multitask_openmp.html), a
...
429. [mxm](mxm/mxm.html), a program which sets up a matrix
    ...
    loops.
430. [mxm\_openmp](mxm_openmp/mxm_openmp.html), a program which computes
    a dense matrix product C=A\*B, using OpenMP for parallel execution.
431. [mxm\_serial](mxm_serial/mxm_serial.html), a program which sets up
    a matrix multiplication problem A=B\*C, intended as a starting point
    for implementing a parallel version.
432. [my\_calendar](my_calendar/my_calendar.html), a program which
    checks a calendar file and prints lines containing the current date;
433. [naca](naca/naca.html), a library which can take the parameters of
    ...
    gnuplot.
434. [nas](nas/nas.html), a program which runs the NASA kernel
    benchmark.
435. [navier\_stokes\_2d\_exact](navier_stokes_2d_exact/navier_stokes_2d_exact.html),
    ...
    2D.
436. [navier\_stokes\_3d\_exact](navier_stokes_3d_exact/navier_stokes_3d_exact.html),
    a library which evaluates an exact solution to the incompressible
    time-dependent Navier-Stokes equations over an arbitrary domain in
    3D.
437. [nearest\_interp\_1d](nearest_interp_1d/nearest_interp_1d.html), a
...
438. [neighbors\_to\_metis\_graph](neighbors_to_metis_graph/neighbors_to_metis_graph.html),
...
439. [netcdf](netcdf/netcdf.html), programs which read and write netcdf
    files.
440. [netcdf\_mpas](netcdf_mpas/netcdf_mpas.html) a library which reads
    MPAS grid files, stored using the NETCDF format.
441. [newton\_interp\_1d](newton_interp_1d/newton_interp_1d.html), a
    library which finds a polynomial interpolant to data using Newton
    divided differences.
442. [niederreiter](niederreiter/niederreiter.html), a library which
    computes elements of the Niederreiter Quasi Mont Carlo (QMC)
    M-dimensional sequence \[arbitrary base\];
443. [niederreiter2](niederreiter2/niederreiter2.html), a library which
    computes elements of the Niederreiter M-dimensional Quasi Mont Carlo
    (QMC) sequence using base 2;
444. [niederreiter2\_dataset](niederreiter2_dataset/niederreiter2_dataset.html),
    a program which creates a Niederreiter Quasi Mont Carlo (QMC)
    dataset with base 2;
445. [nint\_exactness\_mixed](nint_exactness_mixed/nint_exactness_mixed.html),
    a program which measures the polynomial exactness of an
    M-dimensional quadrature rule based on a mixture of 1D quadrature
    rule factors.
446. [nintlib](nintlib/nintlib.html), a library which implements
    approximate integration (quadrature) in M dimensions;
447. [normal](normal/normal.html), a library which implements a random
    number generator (RNG) for normally distributed values.
448. [normal\_dataset](normal_dataset/normal_dataset.html), a program
    which creates a multivariate normal random dataset;
449. [obj\_io](obj_io/obj_io.html), a library which reads and writes the
    data in an OBJ 3D graphics file;
450. [ode](ode/ode.html), a library which solves a system of ordinary
    differential equations, by Shampine and Gordon;
451. [opengl](opengl/opengl.html), programs which illustrate the use of
    the OpenGL graphics library;
452. [openmp](openmp/openmp.html), programs which illustrate the use of
    the OpenMP interface for parallel programming shared memory system;
453. [openmp\_rcc](openmp_rcc/openmp_rcc.html), programs which
...
454. [openmp\_stubs](openmp_stubs/openmp_stubs.html), a library which is
...
455. [optimize](optimize/optimize.html), a program which looks at the
    use of automatic optimization by the compiler to make the executable
    code run faster.
456. [ornstein\_uhlenbeck](ornstein_uhlenbeck/ornstein_uhlenbeck.html),
...
457. [owens](owens/owens.html), a library which evaluates Owen's T
    function;
458. [padua](padua/padua.html), a library which returns the coordinates
    ...
    files.
459. [partial\_digest](partial_digest/partial_digest.html), a library
    which solves the partial digest problem;
460. [partition\_problem](partition_problem/partition_problem.html), a
    library which seeks solutions of the partition problem, splitting a
    set of integers into two subsets with equal sum.
461. [patterson\_rule](patterson_rule/patterson_rule.html), a program
    which returns the points and weights of a 1D Gauss-Patterson
    quadrature rule of order 1, 3, 7, 15, 31, 63, 127, 255 or 511.
462. [pbma\_io](pbma_io/pbma_io.html), a library which reads or writes
    an ASCII Portable Bit Map (PBM) graphics file;
463. [pbma\_to\_pbmb](pbma_to_pbmb/pbma_to_pbmb.html), a program which
    converts an ASCII Portable Bit Map (PBM) file to a binary PBM file;
464. [pbmb\_io](pbmb_io/pbmb_io.html), a library which reads or writes a
    binary Portable Bit Map (PBM) graphics file;
465. [pbmb\_to\_pbma](pbmb_to_pbma/pbmb_to_pbma.html), a program which
    converts a binary Portable Bit Map (PBM) file to an ASCII PBM file;
466. [pce\_burgers](pce_burgers/pce_burgers.html), a program which
...
467. [pce\_ode\_hermite](pce_ode_hermite/pce_ode_hermite.html), a
...
468. [pdflib](pdflib/pdflib.html), a library which evaluates Probability
...
469. [pgma\_io](pgma_io/pgma_io.html), a library which reads or writes
    an ASCII Portable Gray Map (PGM) 2D graphics file;
470. [pgma\_to\_pgmb](pgma_to_pgmb/pgma_to_pgmb.html), a program which
    converts an ASCII Portable Gray Map (PGM) file to a binary PGM file;
471. [pgmb\_io](pgmb_io/pgmb_io.html), a library which reads or writes a
    binary Portable Gray Map (PGM) 2D graphics file;
472. [pgmb\_to\_pgma](pgmb_to_pgma/pgmb_to_pgma.html), a program which
    converts a binary Portable Gray Map (PGM) file to an ASCII PGM file;
473. [piecewise\_linear\_product\_integral](piecewise_linear_product_integral/piecewise_linear_product_integral.html),
    a library which calculates the exact value of the integral of the
    product of two piecewise linear functions f(x) and g(x).
474. [pink\_noise](pink_noise/pink_noise.html), a library which computes
    a pink noise signal obeying a 1/f power law.
475. [point\_merge](point_merge/point_merge.html), a library which
    considers N points in M dimensional space, and counts or indexes the
    unique or tolerably unique items.
476. [poisson\_openmp](poisson_openmp/poisson_openmp.html), a program
...
477. [poisson\_serial](poisson_serial/poisson_serial.html), a program
    which computes an approximate solution to the Poisson equation in a
    rectangle, and is intended as the starting point for the creation of
    a parallel version.
478. [poisson\_simulation](poisson_simulation/poisson_simulation.html),
    ...
    files for processing by gnuplot.
479. [polpak](polpak/polpak.html), a library which evaluates a variety
...
480. [polygon\_grid](polygon_grid/polygon_grid.html), a library which
    generates a grid of points over the interior of a polygon in 2D.
481. [polygon\_integrals](polygon_integrals/polygon_integrals.html), a
    library which returns the exact value of the integral of any
    monomial over the interior of a polygon in 2D.
482. [polygon\_monte\_carlo](polygon_monte_carlo/polygon_monte_carlo.html),
    a library which applies a Monte Carlo method to estimate the
    integral of a function over the interior of a polygon in 2D.
483. [polygon\_properties](polygon_properties/polygon_properties.html),
...
484. [polygon\_triangulate](polygon_triangulate/polygon_triangulate.html),
...
500. [product\_rule](product_rule/product_rule.html), a program which
    creates an M-dimensional quadrature rule from a product of distinct
    1d quadrature rules;
501. [pthreads](pthreads/pthreads.html), programs which illustrate the
    use of the POSIX thread library to carry out parallel program
    execution.
502. [pwl\_approx\_1d](pwl_approx_1d/pwl_approx_1d.html), a library
    which approximates a set of data using a piecewise linear function.
503. [pwl\_interp\_1d](pwl_interp_1d/pwl_interp_1d.html), a library
    which interpolates a set of data using a piecewise linear function,
    creating graphics files for processing by gnuplot.
504. [pwl\_interp\_2d](pwl_interp_2d/pwl_interp_2d.html), a library
    which evaluates a piecewise linear interpolant to data defined on a
    regular 2D grid.
505. [pwl\_interp\_2d\_scattered](pwl_interp_2d_scattered/pwl_interp_2d_scattered.html),
...
522. [quadrature\_test](quadrature_test/quadrature_test.html), a program
    ...
    prints the results.
523. [quadrature\_test\_2d](quadrature_test_2d/quadrature_test_2d.html),
    a program which reads the definition of a 2D quadrature rule from
    three files, applies the rule to a number of test integrals, and
    prints the results.
524. [quadrature\_weights\_vandermonde](quadrature_weights_vandermonde/quadrature_weights_vandermonde.html),
...
525. [quadrature\_weights\_vandermonde\_2d](quadrature_weights_vandermonde_2d/quadrature_weights_vandermonde_2d.html),
    ...
    specified.
526. [quadrule](quadrule/quadrule.html), a library which defines
    quadrature rules for approximating integrals;
527. [quality](quality/quality.html), a library which computes quality
    of distribution measures for pointsets in M dimensions;
528. [r4lib](r4lib/r4lib.html), a library which contains many utility
    routines, using single precision real (R4) arithmetic.
529. [r83](r83/r83.html), a library which contains linear algebra
    routines for R83 matrices (real, 64 bit, tridiagonal 3xN format).
530. [r83\_np](r83_np/r83_np.html), a library which contains linear
    algebra routines for R83\_NP matrices (real, 64 bit, tridiagonal 3xN
    format, nonpivoting factorization).
531. [r83p](r83p/r83p.html), a library which contains linear algebra
    routines for R83P matrices (real, 64 bit, tridiagonal periodic
    format).
532. [r83s](r83s/r83s.html), a library which contains linear algebra
    routines for R83S matrices (real, 64 bit, tridiagonal scalar
    format).
533. [r83t](r83t/r83t.html), a library which contains linear algebra
    routines for R83t matrices (real, 64 bit, tridiagonal Mx3 format).
534. [r83v](r83v/r83v.html), a library which contains linear algebra
    routines for R83V matrices (real, 64 bit, tridiagonal three vector
    format).
535. [r85](r85/r85.html), a library which contains linear algebra
    routines for R85 matrices (real, 64 bit, pentadiagonal format).
536. [r8bb](r8bb/r8bb.html), a library which contains linear algebra
    routines for R8BB matrices (real, 64 bit, border banded format).
537. [r8blt](r8blt/r8blt.html), a library which contains linear algebra
    routines for R8BLT matrices (real, 64 bit, band lower triangular
    format).
538. [r8bto](r8bto/r8bto.html), a library which contains linear algebra
    routines for R8BTO matrices (real, 64 bit, block Toeplitz format).
539. [r8but](r8but/r8but.html), a library which contains linear algebra
    routines for R8BUT matrices (real, 64 bit, band upper triangular
    format).
540. [r8cb](r8cb/r8cb.html), a library which contains linear algebra
    routines for R8CB matrices (real, 64 bit, Compressed Band format).
541. [r8cbb](r8cbb/r8cbb.html), a library which contains linear algebra
    routines for R8CBB matrices (real, 64 bit, Compressed Border Banded
    format).
542. [r8cc](r8cc/r8cc.html), a library which contains linear algebra
    routines for R8CC matrices (real, 64 bit, Compressed Column
    (Colptr,Row,Value) format).
543. [r8ci](r8ci/r8ci.html), a library which contains linear algebra
    routines for R8CI matrices (real, 64 bit, circulant format).
544. [r8col](r8col/r8col.html), a library which contains utility
...
545. [r8gb](r8gb/r8gb.html), a library which contains linear algebra
    routines for R8GB matrices (real, 64 bit, General Banded format).
546. [r8gd](r8gd/r8gd.html), a library which contains linear algebra
    routines for R8GD matrices (real, 64 bit, general diagonal format).
547. [r8ge](r8ge/r8ge.html), a library which contains linear algebra
    routines for R8GE matrices (real, 64 bit, General format).
548. [r8ge\_np](r8ge_np/r8ge_np.html), a library which contains
    nonpivoting linear algebra routines for R8GE\_NP matrices (real, 64
    bit, General nonpivoting format).
549. [r8lib](r8lib/r8lib.html), a library which contains many utility
    routines, using double precision real (R8) arithmetic.
550. [r8lt](r8lt/r8lt.html), a library which contains linear algebra
    routines for R8LT matrices (real, 64 bit, lower triangular format).
551. [r8ltt](r8ltt/r8ltt.html), a library which contains linear algebra
    routines for R8LTT matrices (real, 64 bit, lower triangular Toeplitz
    format).
552. [r8ncf](r8ncf/r8ncf.html), a library which contains linear algebra
    routines for R8NCF matrices (real, 64 bit, nonsymmetric coordinate
    format).
553. [r8pbl](r8pbl/r8pbl.html), a library which contains linear algebra
    routines for R8PBL matrices (real, 64 bit, positive definite
    symmetric band lower format).
554. [r8pbu](r8pbu/r8pbu.html), a library which contains linear algebra
    routines for R8PBU matrices (real, 64 bit, positive definite
    symmetric band upper format).
555. [r8po](r8po/r8po.html), a library which contains linear algebra
    routines for R8PO matrices (real, 64 bit, positive definite
    symmetric format).
556. [r8pp](r8pp/r8pp.html), a library which contains linear algebra
    routines for R8PP matrices (real, 64 bit, positive definite
    symmetric packed format).
557. [r8ri](r8ri/r8ri.html), a library which contains linear algebra
    routines for R8RI matrices (real, 64 bit, row-indexed format).
558. [r8row](r8row/r8row.html), a library which contains utility
    ...
    carried out rowwise.
559. [r8s3](r8s3/r8s3.html), a library which contains linear algebra
    routines for R8S3 matrices (real, 64 bit, SLAP triad (I,J,AIJ)
    format).
560. [r8sd](r8sd/r8sd.html), a library which contains linear algebra
    routines for R8SD matrices (real, 64 bit, symmetric diagonal
    format).
561. [r8sm](r8sm/r8sm.html), a library which contains linear algebra
    routines for R8SM (Sherman-Morrison) matrices (real, 64 bit, A-u\*v'
    format).
562. [r8sp](r8sp/r8sp.html), a library which contains linear algebra
    routines for R8SP matrices (real, 64 bit, SLAP triad (I,J,AIJ)
    format).
563. [r8sr](r8sr/r8sr.html), a library which carries out linear algebra
    operations for R8SR matrices (real, 64 bit, diagonal + compressed
    row offdiagonal format).
564. [r8ss](r8ss/r8ss.html), a library which carries out linear algebra
    operations for R8SS matrices (real, 64 bit, symmetric skyline
    format).
565. [r8sto](r8sto/r8sto.html), a library which contains linear algebra
    routines for R8STO matrices (real, 64 bit, symmetric Toeplitz N
    format).
566. [r8to](r8to/r8to.html), a library which contains linear algebra
    routines for R8TO matrices (real, 64 bit, Toeplitz 2\*N-1 format).
567. [r8ut](r8ut/r8ut.html), a library which contains linear algebra
    routines for R8UT matrices (real, 64 bit, upper triangular format).
568. [r8utt](r8utt/r8utt.html), a library which contains linear algebra
    routines for R8UTT matrices (real, 64 bit, upper triangular Toeplitz
    format).
569. [r8vm](r8vm/r8vm.html), a library which contains linear algebra
    routines for R8VM matrices (real, 64 bit, Vandermonde format).
570. [rand48](rand48/rand48.html), a program which demonstrates the use
    of the RAND48 family of random number generators (RNG's) available
    in the C/C++ standard library.
571. [randlc](randlc/randlc.html), a library which implements a random
    number generator (RNG) used by the NAS Benchmark programs.
572. [random\_data](random_data/random_data.html), a library which uses
...
573. [random\_mpi](random_mpi/random_mpi.html), a program which
...
574. [random\_openmp](random_openmp/random_openmp.html), a program which
    illustrates how a parallel program using OpenMP can generate
    multiple distinct streams of random numbers.
575. [random\_sorted](random_sorted/random_sorted.html), a library which
    generates vectors of random values which are already sorted.
576. [ranlib](ranlib/ranlib.html), a library which produces random
...
577. [rbf\_interp\_1d](rbf_interp_1d/rbf_interp_1d.html), a library
    which defines and evaluates radial basis function (RBF) interpolants
    to 1D data.
578. [rbf\_interp\_2d](rbf_interp_2d/rbf_interp_2d.html), a library
    which defines and evaluates radial basis function (RBF) interpolants
    to 2D data.
579. [rbf\_interp\_nd](rbf_interp_nd/rbf_interp_nd.html), a library
    which defines and evaluates radial basis function (RBF) interpolants
    to M-dimensional data.
580. [rcm](rcm/rcm.html), a library which applies the Reverse Cuthill
    McKee (RCM) algorithm for reordering the nodes of a graph, and
    reducing the bandwidth of a corresponding sparse matrix;
581. [reactor simulation](reactor_simulation/reactor_simulation.html), a
...
594. [sandia\_rules2](sandia_rules2/sandia_rules2.html), a library which
...
595. [sandia\_sgmg](sandia_sgmg/sandia_sgmg.html), a library which
...
596. [sandia\_sgmga](sandia_sgmga/sandia_sgmga.html), a library which
...
597. [sandia\_sgmgg](sandia_sgmgg/sandia_sgmgg.html), a library which
    explores a generalized construction method for sparse grids.
598. [sandia\_sparse](sandia_sparse/sandia_sparse.html), a library which
...
599. [satisfy](satisfy/satisfy.html), a program which demonstrates, for
    a particular circuit, an exhaustive search for solutions of the
    circuit satisfy problem.
600. [satisfy\_mpi](satisfy_mpi/satisfy_mpi.html), a program which
...
601. [satisfy\_openmp](satisfy_openmp/satisfy_openmp.html), a program
    ...
    parallel execution.
602. [schedule\_openmp](schedule_openmp/schedule_openmp.html), a program
...
603. screenshot\_opengl, a program which shows how a program using the
    ...
    displayed.
604. [scvt\_mpi](scvt_mpi/scvt_mpi.html), a program which determines a
...
605. [sde](sde/sde.html), a library which illustrates properties of
...
606. [search\_mpi](search_mpi/search_mpi.html), a program which searches
    integers between A and B for a solution J such that F(J)=C, carried
    out in parallel using MPI.
607. [search\_serial](search_serial/search_serial.html), a program which
...
620. [simplex\_grid](simplex_grid/simplex_grid.html), a library which
    generates a regular grid of points over the interior of an arbitrary
    simplex in M dimensions.
621. [simplex\_integrals](simplex_integrals/simplex_integrals.html), a
    library which returns the exact value of the integral of any
    monomial over the interior of the unit simplex in M dimensions.
622. [simplex\_monte\_carlo](simplex_monte_carlo/simplex_monte_carlo.html),
    a library which uses the Monte Carlo method to estimate an integral
    over the interior of the unit simplex in M dimensions.
623. [sine\_transform](sine_transform/sine_transform.html), a library
    which demonstrates some simple properties of the discrete sine
    transform.
624. [snakes\_and\_ladders](snakes_and_ladders/snakes_and_ladders.html),
    programs which simulate and investigate a one-player version of the
    game of Snakes and Ladders.
625. [sobol](sobol/sobol.html), a library which computes elements of the
    Sobol M-dimensional Quasi Mont Carlo (QMC) sequence, extended to a
    maximal spatial dimension of 1111, by Bennett Fox.
626. [sobol\_dataset](sobol_dataset/sobol_dataset.html), a program which
    creates a Sobol Quasi Mont Carlo (QMC) dataset;
627. [solve](solve/solve.html), a library which implements a linear
    solver which makes it easy to create doubly-dimensioned arrays and
    solve associated linear systems.
628. [sor](sor/sor.html), a library which implements a simple version of
    the Successive Over-Relaxation (SOR) method for the iterative
    solution of a linear system of equations.
629. [sort\_rc](sort_rc/sort_rc.html), a library which can sort a list
    of any kind of objects, using reverse communication (RC).
630. [sparse\_count](sparse_count/sparse_count.html), a library which
...
631. [sparse\_display](sparse_display/sparse_display.html), a library
    ...
    "spy" command.
632. [sparse\_grid\_cc](sparse_grid_cc/sparse_grid_cc.html), a library
    which creates sparse grids based on Clenshaw-Curtis rules.
633. [sparse\_grid\_cc\_dataset](sparse_grid_cc_dataset/sparse_grid_cc_dataset.html),
    a program which creates a sparse grid dataset based on
    Clenshaw-Curtis rules.
634. [sparse\_grid\_gl](sparse_grid_gl/sparse_grid_gl.html), a library
    which creates sparse grids based on Gauss-Legendre rules.
635. [sparse\_grid\_gl\_dataset](sparse_grid_gl_dataset/sparse_grid_gl_dataset.html),
    a program which creates a sparse grid dataset based on
    Gauss-Legendre rules.
636. [sparse\_grid\_hermite](sparse_grid_hermite/sparse_grid_hermite.html),
    a library which creates sparse grids based on Gauss-Hermite rules.
637. [sparse\_grid\_hermite\_dataset](sparse_grid_hermite_dataset/sparse_grid_hermite_dataset.html),
    a program which creates a sparse grid dataset based on Gauss-Hermite
    rules.
638. [sparse\_grid\_hw](sparse_grid_hw/sparse_grid_hw.html), a library
...
639. [sparse\_grid\_laguerre](sparse_grid_laguerre/sparse_grid_laguerre.html),
    a library which creates sparse grids based on Gauss-Laguerre rules.
640. [sparse\_grid\_laguerre\_dataset](sparse_grid_laguerre_dataset/sparse_grid_laguerre_dataset.html),
    a program which creates a sparse grid dataset based on
    Gauss-Laguerrre rules.
641. [sparse\_grid\_mixed](sparse_grid_mixed/sparse_grid_mixed.html), a
    library which creates sparse grids based on a mixture of 1D factor
    rules.
642. [sparse\_grid\_mixed\_dataset](sparse_grid_mixed_dataset/sparse_grid_mixed_dataset.html),
    a program which creates a sparse grid dataset based on a mixture of
    1D rules.
643. [sparse\_grid\_open](sparse_grid_open/sparse_grid_open.html), a
    library which creates sparse grids based on open rules (Fejer 2,
    Gauss-Patterson, Newton-Cotes-Open).
644. [sparse\_grid\_open\_dataset](sparse_grid_open_dataset/sparse_grid_open_dataset.html),
    a program which creates a sparse grid dataset based on open rules
    (Fejer 2, Gauss-Patterson, Newton-Cotes-Open).
645. [sparse\_interp\_nd](sparse_interp_nd/sparse_interp_nd.html), a
    library which can be used to define a sparse interpolant to a
    function f(x) of a M-dimensional argument.
646. [sphere\_exactness](sphere_exactness/sphere_exactness.html), a
    program which tests the polynomial exactness of a quadrature rule
    over the surface of the unit sphere in 3D;
647. [sphere\_fibonacci\_grid](sphere_fibonacci_grid/sphere_fibonacci_grid.html),
    a library which uses a Fibonacci spiral to create a grid of points
    on the surface of the unit sphere in 3D.
648. [sphere\_grid](sphere_grid/sphere_grid.html), a library which
...
649. [sphere\_integrals](sphere_integrals/sphere_integrals.html), a
    library which returns the exact value of the integral of any
    monomial over the surface of the unit sphere in 3D.
650. [sphere\_lebedev\_rule](sphere_lebedev_rule/sphere_lebedev_rule.html),
    a library which computes Lebedev quadrature rules for the surface of
    the unit sphere in 3D;
651. [sphere\_llq\_grid](sphere_llq_grid/sphere_llq_grid.html), a
    ...
    in 3D.
652. [sphere\_llt\_grid](sphere_llt_grid/sphere_llt_grid.html), a
    library which uses longitudes and latitudes to create grids of
    points, lines, and triangles on the surface of the unit sphere in
    3D.
653. [sphere\_monte\_carlo](sphere_monte_carlo/sphere_monte_carlo.html),
    a library which applies a Monte Carlo method to estimate the
    integral of a function over the surface of the unit sphere in 3D;
654. [sphere\_quad](sphere_quad/sphere_quad.html), a library which uses
    triangulation to approximate an integral over the surface of the
    unit sphere in 3D;
655. [sphere\_stereograph](sphere_stereograph/sphere_stereograph.html),
...
656. [sphere\_triangle\_monte\_carlo](sphere_triangle_monte_carlo/sphere_triangle_monte_carlo.html),
    ...
    sphere in 3D.
657. [sphere\_triangle\_quad](sphere_triangle_quad/sphere_triangle_quad.html),
    ...
    in 3D.
658. [sphere\_voronoi\_display\_opengl](sphere_voronoi_display_opengl/sphere_voronoi_display_opengl.html),
...
659. [sphere\_xyz\_display\_opengl](sphere_xyz_display_opengl/sphere_xyz_display_opengl.html),
...
660. [spiral\_data](spiral_data/spiral_data.html), a library which
...
661. [spline](spline/spline.html), a library which interpolates or
    approximates data by piecewise polynomial functions;
662. [spring\_ode](spring_ode/spring_ode.html), a program which shows
...
663. [spring\_ode2](spring_ode2/spring_ode2.html), a program which shows
...
664. [square\_arbq\_rule](square_arbq_rule/square_arbq_rule.html), a
...
665. [square\_exactness](square_exactness/square_exactness.html), a
    library which investigates the polynomial exactness of quadrature
    rules for f(x,y) over the interior of a square in 2D.
666. [square\_felippa\_rule](square_felippa_rule/square_felippa_rule.html),
    a library which returns Felippa's quadratures rules for
    approximating integrals over the interior of a square in 2D.
667. [square\_grid](square_grid/square_grid.html), a library which
    computes a grid of points over the interior of a square in 2D.
668. [square\_integrals](square_integrals/square_integrals.html), a
    library which returns the exact value of the integral of any
    monomial over the interior of the unit square in 2D.
669. [square\_monte\_carlo](square_monte_carlo/square_monte_carlo.html),
    a library which applies a Monte Carlo method to estimate the
    integral of a function over the interior of the unit square in 2D.
670. [square\_symq\_rule](square_symq_rule/square_symq_rule.html), a
...
671. [st\_io](st_io/st_io.html), a library which reads and writes sparse
    linear systems stored in the Sparse Triplet (ST) format.
672. [st\_to\_cc](st_to_cc/st_to_cc.html), a library which converts
    sparse matrix data from Sparse Triplet (ST) format to Compressed
    Column (CC) format;
673. [star\_discrepancy](star_discrepancy/star_discrepancy.html), a
...
674. [stla\_io](stla_io/stla_io.html), a library which reads and writes
    ASCII stereolithography (STL) 3d graphics files;
675. [stochastic\_diffusion](stochastic_diffusion/stochastic_diffusion.html),
    functions which implement several versions of a stochastic
    diffusivity coefficient.
676. [stochastic\_heat2d](stochastic_heat2d/stochastic_heat2d.html), a
    program which implements a finite difference method (FDM) for the
    steady (time independent) 2D heat equation, with a stochastic heat
    diffusivity coefficient.
677. [stochastic\_rk](stochastic_rk/stochastic_rk.html), a library which
    applies a Runge-Kutta (RK) scheme to a stochastic ordinary
    differential equation (SODE).
678. [stokes\_2d\_exact](stokes_2d_exact/stokes_2d_exact.html), a
    library which evaluates exact solutions to the incompressible steady
    Stokes equations over the unit square in 2D.
679. [string\_simulation](string_simulation/string_simulation.html), a
...
680. [stroud](stroud/stroud.html), a library which defines quadrature
...
681. [subpak](subpak/subpak.html), a library which contains many utility
    routines;
682. [subset](subset/subset.html), a library which enumerates,
    ...
    problems.
683. [subset\_sum](subset_sum/subset_sum.html), a program which seeks
    solutions of the subset sum problem.
684. [subset\_sum\_serial](subset_sum_serial/subset_sum_serial.html), a
    program which seeks solutions of the subset sum problem, intended as
    a starting point for a parallel computing approach.
685. [superlu](superlu/superlu.html), programs which illustrate how a
...
686. [superlu\_openmp](superlu_openmp/superlu_openmp.html), programs
...
687. [svd\_basis](svd_basis/svd_basis.html), a program which applies the
    singular value decomposition (SVD) to a set of vectors, to extract
    the dominant modes;
688. [svd\_demo](svd_demo/svd_demo.html), a program which demonstrates
    the calculation of the singular value decomposition (SVD) and some
    of its properties;
689. [svd\_snowfall](svd_snowfall/svd_snowfall.html), a program which
...
690. [svd\_truncated](svd_truncated/svd_truncated.html), a program which
...
697. [test\_approx](test_approx/test_approx.html), a library which
    implements test problems for approximation, provided as a set of
    (x,y) data.
698. [test\_eigen](test_eigen/test_eigen.html), a library which
    implements test matrices for eigenvalue analysis.
699. [test\_int](test_int/test_int.html), a library which implements
    test problems for approximate integration in one dimension.
700. [test\_int\_2d](test_int_2d/test_int_2d.html), a library which
    implements test problems for approximate integration in two
    dimensions.
701. [test\_interp](test_interp/test_interp.html), a library which
    defines test problems for interpolation, provided as a set of (x,y)
    data.
702. [test\_interp\_1d](test_interp_1d/test_interp_1d.html), a library
    which defines test problems for interpolation of data y(x), which
    depends on a 1D argument.
703. [test\_interp\_2d](test_interp_2d/test_interp_2d.html), a library
    which defines test problems for interpolation of data z(x,y),
    depending on a 2D argument.
704. [test\_interp\_nd](test_interp_nd/test_interp_nd.html), a library
    which defines test problems for interpolation of data z(x),
    depending on an M-dimensional argument.
705. [test\_lls](test_lls/test_lls.html), a library which implements
    linear least squares (LLS) test problems of the form A\*x=b.
706. [test\_mat](test_mat/test_mat.html), a library which defines test
...
707. [test\_matrix\_exponential](test_matrix_exponential/test_matrix_exponential.html),
    a library which defines a set of test cases for computing the matrix
    exponential.
708. [test\_min](test_min/test_min.html), a library which implements
    test problems for minimization of a scalar function of a scalar
    variable.
709. [test\_nearest](test_nearest/test_nearest.html), a library which
    tests the time complexity of various procedures for solving the
    nearest neighbor problem.
710. [test\_nint](test_nint/test_nint.html), a library which implements
    test problems for approximate integration (quadrature) in M
    dimensions.
711. [test\_opt\_con](test_opt_con/test_opt_con.html), a library which
...
712. [test\_optimization](test_optimization/test_optimization.html), a
...
713. [test\_partial\_digest](test_partial_digest/test_partial_digest.html),
    a library which can generate example cases of the partial digest
    problem.
714. [test\_values](test_values/test_values.html), a library which
...
715. [test\_zero](test_zero/test_zero.html), a library which implements
    test problems for the solution of a single nonlinear equation in one
    variable.
716. [testpack](testpack/testpack.html), a program which demonstrates
    the use of a set of test integrand functions chosen by Alan Genz.
717. [tet\_mesh](tet_mesh/tet_mesh.html), a library which works with
    tetrahedral meshes in 3D;
718. [tet\_mesh\_boundary](tet_mesh_boundary/tet_mesh_boundary.html), a
...
727. [tet\_mesh\_volumes](tet_mesh_volumes/tet_mesh_volumes.html), a
    program which computes the volume of each tetrahedron in a tet mesh;
728. [tetrahedron\_arbq\_rule](tetrahedron_arbq_rule/tetrahedron_arbq_rule.html),
...
729. [tetrahedron\_exactness](tetrahedron_exactness/tetrahedron_exactness.html),
    a program which tests the polynomial exactness of a quadrature rule
    over the interior of a tetrahedron in 3D.
730. [tetrahedron\_felippa\_rule](tetrahedron_felippa_rule/tetrahedron_felippa_rule.html),
    a library which returns Felippa's quadratures rules for
    approximating integrals over the interior of a tetrahedron in 3D.
731. [tetrahedron\_grid](tetrahedron_grid/tetrahedron_grid.html), a
    library which computes a grid of points over the interior of a
    tetrahedron in 3D.
732. [tetrahedron\_integrals](tetrahedron_integrals/tetrahedron_integrals.html),
    a library which returns the exact value of the integral of any
    monomial over the interior of the unit tetrahedron in 3D.
733. [tetrahedron\_keast\_rule](tetrahedron_keast_rule/tetrahedron_keast_rule.html),
...
734. [tetrahedron\_monte\_carlo](tetrahedron_monte_carlo/tetrahedron_monte_carlo.html),
    a library which uses the Monte Carlo method to estimate an integral
    over the interior of the unit tetrahedron in 3D.
735. [tetrahedron\_ncc\_rule](tetrahedron_ncc_rule/tetrahedron_ncc_rule.html),
    a library which defines Newton-Cotes closed (NCC) quadrature rules
    over the interior of a tetrahedron in 3D.
736. [tetrahedron\_nco\_rule](tetrahedron_nco_rule/tetrahedron_nco_rule.html),
    a library which defines Newton-Cotes open (NCO) quadrature rules
    over the interior of a tetrahedron in 3D.
737. [tetrahedron\_properties](tetrahedron_properties/tetrahedron_properties.html),
    ...
    in 3D.
738. [three\_body\_simulation](three_body_simulation/three_body_simulation.html),
...
739. [tiler\_3d](tiler_3d/tiler_3d.html), a library which blends a
    pattern repeatedly into a region in 3D;
740. [timer](timer/timer.html), programs which measure the CPU time used
    by a portion of a user's program;
741. [timestamp](timestamp/timestamp.html), a library which prints the
    current YMDHMS date as a timestamp, and other timekeeping
    operations;
742. [toeplitz\_cholesky](toeplitz_cholesky/toeplitz_cholesky.html), a
    library which computes the Cholesky factorization of a nonnegative
    definite symmetric Toeplitz matrix.
743. [toms097](toms097/toms097.html), a library which computes the
    ...
    TOMS algorithm 97.
744. [toms112](toms112/toms112.html), a library which determines whether
    a point is contained in a polygon, by Moshe Shimrat. This is a
    version of ACM TOMS algorithm 112.
745. [toms178](toms178/toms178.html), a library which seeks the
    ...
    algorithm 178.
746. [toms179](toms179/toms179.html), a library which calculates the
    incomplete Beta ratio, by Oliver Ludwig. This is a version of ACM
    TOMS algorithm 179.
747. [toms291](toms291/toms291.html), a library which approximates the
    logarithm of the Gamma function; this is a version of ACM TOMS
    algorithm 291.
748. [toms443](toms443/toms443.html), a library which evaluates
    Lambert's W function. This is a version of ACM TOMS algorithm 443.
749. [toms446](toms446/toms446.html), a library which manipulates
    Chebyshev series for interpolation and approximation. This is a
    version of ACM TOMS algorithm 446, by Roger Broucke.
750. [toms462](toms462/toms462.html), a library which evaluates the
    ...
    algorithm 462.
751. [toms515](toms515/toms515.html), a library which can select subsets
    of size K from a set of size N, ACM TOMS Algorithm 515, by Bill
    Buckles, Matthew Lybanon.
752. [toms655](toms655/toms655.html), a library which computes the
...
753. [toms743](toms743/toms743.html), a library which evaluates
    Lambert's W function. This is a version of ACM TOMS algorithm 743,
    by Barry, Barry and Culligan-Hensley.
754. [toms886](toms886/toms886.html), a library which defines the Padua
...
755. [toms917](toms917/toms917.html), a library which evaluates the
    Wright Omega function, by Piers Lawrence, Robert Corless, David
    Jeffrey. This is a version of ACM TOMS algorithm 917.
756. [treepack](treepack/treepack.html), a library which carries out
    computations on trees, a simple kind of graph that is minimally
    connected.
757. [tri\_surface\_display\_opengl](tri_surface_display_opengl/tri_surface_display_opengl.html),
    a program which displays a 3D triangulated surface or TRI\_SURFACE
    using OpenGL;
758. [tri\_surface\_io](tri_surface_io/tri_surface_io.html), a library
    which reads and writes the 3D graphics information in a tri\_surface
    file;
759. [triangle\_analyze](triangle_analyze/triangle_analyze.html), a
    ...
    quality.
760. [triangle\_dunavant\_rule](triangle_dunavant_rule/triangle_dunavant_rule.html),
    a library which defines Dunavant quadrature rules over the interior
    of a triangle in 2D.
761. [triangle\_exactness](triangle_exactness/triangle_exactness.html),
    a program which tests the polynomial exactness of a quadrature rule
    over the interior of a triangle in 2D.
762. [triangle\_fekete\_rule](triangle_fekete_rule/triangle_fekete_rule.html),
    a library which sets up one of seven Fekete rules for interpolation
    or quadrature over the interior of a triangle in 2D.
763. [triangle\_felippa\_rule](triangle_felippa_rule/triangle_felippa_rule.html),
    a library which returns Felippa's quadratures rules for
    approximating integrals over the interior of a triangle in 2D.
764. [triangle\_grid](triangle_grid/triangle_grid.html), a library which
    computes a grid of points over the interior of a triangle in 2D.
765. [triangle\_histogram](triangle_histogram/triangle_histogram.html),
    a program which computes histograms of data over the interior of the
    unit triangle in 2D.
766. [triangle\_integrals](triangle_integrals/triangle_integrals.html),
    a library which returns the exact value of the integral of any
    polynomial over the interior of an arbitrary triangle in 2D.
767. [triangle\_interpolate](triangle_interpolate/triangle_interpolate.html),
    a library which shows how vertex data can be interpolated at any
    point in the interior of a triangle.
768. [triangle\_io](triangle_io/triangle_io.html), a library which can
    read or write some of the files created by Jonathan Shewchuk's
    triangle program.
769. [triangle\_lyness\_rule](triangle_lyness_rule/triangle_lyness_rule.html),
    a library which returns Lyness-Jespersen quadrature rules over the
    interior of a triangle in 2D;
770. [triangle\_monte\_carlo](triangle_monte_carlo/triangle_monte_carlo.html),
    a library which uses the Monte Carlo method to estimate an integral
    over the interior of a triangle in 2D.
771. [triangle\_ncc\_rule](triangle_ncc_rule/triangle_ncc_rule.html), a
    library which defines Newton-Cotes closed (NCC) quadrature rules
    over the interior of a triangle in 2D.
772. [triangle\_nco\_rule](triangle_nco_rule/triangle_nco_rule.html), a
    library which defines Newton-Cotes open (NCO) quadrature rules over
    the interior of a triangle in 2D.
773. [triangle\_properties](triangle_properties/triangle_properties.html),
...
774. [triangle\_svg](triangle_svg/triangle_svg.html), a library which
    uses Scalable Vector Graphics (SVG) to plot a triangle and any
    number of points, to illustrate quadrature rules and sampling
    techniques.
775. [triangle\_symq\_rule](triangle_symq_rule/triangle_symq_rule.html),
...
776. [triangle\_to\_fem](triangle_to_fem/triangle_to_fem.html), a
...
777. [triangle\_to\_medit](triangle_to_medit/triangle_to_medit.html), a
    program which reads the NODE and ELE files created by the triangle()
    program to describe a triangular mesh, and writes a corresponding
    MESH file for input to medit().
778. [triangle\_to\_xml](triangle_to_xml/triangle_to_xml.html), a
...
779. [triangle\_wandzura\_rule](triangle_wandzura_rule/triangle_wandzura_rule.html),
    a library which returns quadrature rules of exactness 5, 10, 15, 20,
    25 and 30 over the interior of the triangle in 2D.
780. [triangle01\_integrals](triangle01_integrals/triangle01_integrals.html),
    a library which returns the integral of any monomial over the
    interior of the unit triangle in 2D.
781. [triangulation](triangulation/triangulation.html), a library which
    ...
    given point.
782. [triangulation\_boundary\_nodes](triangulation_boundary_nodes/triangulation_boundary_nodes.html),
    ...
    file;
783. [triangulation\_corner](triangulation_corner/triangulation_corner.html),
    a program which modifies triangulations in which one or more
    triangles have more than one boundary edge.
784. [triangulation\_delaunay\_discrepancy](triangulation_delaunay_discrepancy/triangulation_delaunay_discrepancy.html),
    a program which measures the amount by which a triangulation fails
    the local Delaunay test;
785. [triangulation\_display\_opengl](triangulation_display_opengl/triangulation_display_opengl.html),
    a program which reads two files describing a triangulation and
    displays it using OpenGL.
786. [triangulation\_histogram](triangulation_histogram/triangulation_histogram.html),
    a program which computes histograms of data over a triangulation.
787. [triangulation\_l2q](triangulation_l2q/triangulation_l2q.html), a
...
788. [triangulation\_mask](triangulation_mask/triangulation_mask.html),
    a program which reads a triangulation and calls a user-supplied
    routine to consider each triangle for deletion;
789. [triangulation\_node\_to\_element](triangulation_node_to_element/triangulation_node_to_element.html),
    ...
    the data.
790. [triangulation\_orient](triangulation_orient/triangulation_orient.html),
    a program which ensures that the triangles in an order 3 or order 6
    triangulation have positive orientation;
791. [triangulation\_plot](triangulation_plot/triangulation_plot.html),
    a program which plots the nodes and elements of a triangulation as a
    PostScript (PS) file;
792. [triangulation\_q2l](triangulation_q2l/triangulation_q2l.html), a
...
793. [triangulation\_quad](triangulation_quad/triangulation_quad.html),
...
794. [triangulation\_quality](triangulation_quality/triangulation_quality.html),
    a program which reads information about a triangulation and computes
    various quality measures;
795. [triangulation\_rcm](triangulation_rcm/triangulation_rcm.html), a
    ...
    triangles;
796. [triangulation\_refine](triangulation_refine/triangulation_refine.html),
    a program which refines a triangulation;
797. [triangulation\_svg](triangulation_svg/triangulation_svg.html), a
    program which creates an SVG image of a triangulation, which can be
    displayed by a web browser.
798. [triangulation\_t3\_to\_t4](triangulation_t3_to_t4/triangulation_t3_to_t4.html),
    ...
    (vertices + centroid);
799. [triangulation\_triangle\_neighbors](triangulation_triangle_neighbors/triangulation_triangle_neighbors.html),
...
800. [truncated\_normal](truncated_normal/truncated_normal.html), a
...
801. [truncated\_normal\_rule](truncated_normal_rule/truncated_normal_rule.html),
...
802. [tsg](tsg/tsg.html), programs which demonstrate the use of the
...
803. [tsp\_brute](tsp_brute/tsp_brute.html), a program which reads a
    file of city-to-city distances and solves a (small!) traveling
    salesperson problem, using brute force.
804. [ubvec](ubvec/ubvec.html), a library which demonstrates how
    nonnegative integers can be stored as unsigned binary vectors, and
    arithmetic can be performed on them.
805. [umfpack](umfpack/umfpack.html), examples which illustrate how to
    solve a sparse linear system by calling the C library UMFPACK, by
    Timothy Davis.
806. [uncontrol](uncontrol/uncontrol.html), a program which removes
    control characters from a file;
807. [unicycle](unicycle/unicycle.html), a library which considers
    permutations containing a single cycle, sometimes called cyclic
    permutations.
808. [uniform](uniform/uniform.html), a library which implements uniform
    random number generators (RNG's) for a variety of arithmetic types.
809. [uniform\_dataset](uniform_dataset/uniform_dataset.html), a program
    which creates a file of uniform pseudorandom values;
810. [upc](upc/upc.html), is a library which computes the check digit of
    a uniform product code (UPC), or reports whether a full 12-digit UPC
    is actually valid.
811. [valgrind](valgrind/valgrind.html), programs which illustrate the
    use of the VALGRIND suite of programs which includes a memory leak
    detector.
812. [van\_der\_corput](van_der_corput/van_der_corput.html), a library
    which evaluates the van der Corput 1-dimensional Quasi Mont Carlo
    (QMC) sequence, using a simple interface.
813. [van\_der\_corput\_advanced](van_der_corput_advanced/van_der_corput_advanced.html),
...
814. [van\_der\_corput\_dataset](van_der_corput_dataset/van_der_corput_dataset.html),
    a program which creates a file of Quasi Mont Carlo (QMC) van der
    Corput values;
815. [vandermonde](vandermonde/vandermonde.html), a library which
    implements the Bjork-Pereyra algorithm for accurate solution of
    linear systems involving the Vandermonde matrix.
816. [vandermonde\_approx\_1d](vandermonde_approx_1d/vandermonde_approx_1d.html),
...
817. [vandermonde\_approx\_2d](vandermonde_approx_2d/vandermonde_approx_2d.html),
    ...
    matrix.
818. [vandermonde\_interp\_1d](vandermonde_interp_1d/vandermonde_interp_1d.html),
    ...
    graphics files for processing by gnuplot.
819. [vandermonde\_interp\_2d](vandermonde_interp_2d/vandermonde_interp_2d.html),
...
820. [walker\_sample](walker_sample/walker_sample.html), a library which
    efficiently samples a discrete probability vector using Walker
    sampling.
821. [walsh](walsh/walsh.html), a library which implements versions of
    the Walsh and Haar transforms.
822. [wathen](wathen/wathen.html), a library which compares storage
...
823. [wave\_mpi](wave_mpi/wave_mpi.html), a program which uses finite
    differences and the Message Passing Interface (MPI) to estimate a
    solution to the wave equation.
824. [wavelet](wavelet/wavelet.html), a library which does some simple
    wavelet calculations;
825. [wedge\_exactness](wedge_exactness/wedge_exactness.html), a program
    which investigates the polynomial exactness of a quadrature rule
    over the interior of the unit wedge in 3D.
826. [wedge\_felippa\_rule](wedge_felippa_rule/wedge_felippa_rule.html),
    a library which returns quadrature rules for approximating integrals
    over the interior of the unit wedge in 3D.
827. [wedge\_grid](wedge_grid/wedge_grid.html), a library which computes
    a grid of points over the interior of the unit wedge in 3D.
828. [wedge\_integrals](wedge_integrals/wedge_integrals.html), a library
    which returns the exact value of the integral of any monomial over
    the interior of the unit wedge in 3D.
829. [wedge\_monte\_carlo](wedge_monte_carlo/wedge_monte_carlo.html), a
    library which uses the Monte Carlo method to estimate an integral
    over the interior of the unit wedge in 3D.
830. [weekday](weekday/weekday.html), a library which determines the day
    of the week corresponding to a given date, such as 14 October 1066,
    Julian calendar, ... which was a Saturday.
831. [width](width/width.html), a program which returns the length and
    location of the longest line in a text file;
832. [wishart](wishart/wishart.html), a library which produces sample
    matrices from the Wishart or Bartlett distributions, useful for
    sampling random covariance matrices.
833. [wrap](wrap/wrap.html), a program which wraps a file so that no
    line is longer than 80 characters;
834. [wrap2](wrap2/wrap2.html), a program which wraps a text file so
    ...
    breaking words;
835. [wtime](wtime/wtime.html), a library which returns a reading of the
    wall clock time in seconds.
836. [xy\_display\_opengl](xy_display_opengl/xy_display_opengl.html), a
    program which reads XY information defining points in 2D and
    displays an image using OpenGL;
837. [xy\_io](xy_io/xy_io.html), a library which reads and writes XY,
    XYL and XYF files defining points, lines, and faces in 2D.
838. [xyf\_display\_opengl](xyf_display_opengl/xyf_display_opengl.html),
    a program which reads XYF information defining points and faces in
    2D and displays an image using OpenGL;
839. [xyl\_display\_opengl](xyl_display_opengl/xyl_display_opengl.html),
    a program which reads XYL information defining points and lines in
    2D and displays an image using OpenGL;
840. [xyz\_display\_opengl](xyz_display_opengl/xyz_display_opengl.html),
    a program which reads XYZ information defining points in 3D and
    displays an image using OpenGL;
841. [xyz\_io](xyz_io/xyz_io.html), a library which reads and writes
    XYZ, XYZL and XYZF files defining points, lines and faces in 3D.
842. [xyzf\_display\_opengl](xyzf_display_opengl/xyzf_display_opengl.html),
    a program which reads XYZF information defining points and faces in
    3D and displays an image using OpenGL;
843. [xyzl\_display\_opengl](xyzl_display_opengl/xyzl_display_opengl.html),
    a program which reads XYZL information defining points and lines in
    3D and displays an image using OpenGL;
844. [zero\_rc](zero_rc/zero_rc.html), a library which seeks a solution
    of a scalar nonlinear equation f(x)=0, using reverse communication
    (RC), by Richard Brent.
845. [ziggurat](ziggurat/ziggurat.html), a library which samples the
    uniform, normal or exponential distributions, using the ziggurat
    method, by Marsaglia and Tsang.
846. [ziggurat\_inline](ziggurat_inline/ziggurat_inline.html), a library
    ...
    for increased speed.
847. [ziggurat\_openmp](ziggurat_openmp/ziggurat_openmp.html), a program
