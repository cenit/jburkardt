R8SP\
SLAP Triad Matrix Utilities {#r8sp-slap-triad-matrix-utilities align="center"}
===========================

------------------------------------------------------------------------

**R8SP** is a C++ library which carries out some basic linear algebra
operations on R8SP matrices (real 64 bit precision SLAP Triad (I,J,AIJ)
format).
...
**R8SP** is available in [a C version](../../c_src/r8sp/r8sp.html) and
[a C++ version](../../cpp_src/r8sp/r8sp.html) and [a FORTRAN90
version](../../f_src/r8sp/r8sp.html) and [a MATLAB
version](../../m_src/r8sp/r8sp.html) and [a Python
version.](../../py_src/r8sp/r8sp.html)
...
-   [r8sp.cpp](r8sp.cpp), the source code.
-   [r8sp.hpp](r8sp.hpp), the source code.

### Examples and Tests: {#examples-and-tests align="center"}

-   [r8sp\_prb.cpp](r8sp_prb.cpp), a sample calling program.
-   [r8sp\_prb\_output.txt](r8sp_prb_output.txt), the output file.
-   [r8sp\_matrix.txt](r8sp_matrix.txt), a text file storing an example
    of an R8SP matrix.
...
-   **R8NCF\_PRINT** prints an R8NCF matrix.
-   **R8NCF\_PRINT\_SOME** prints some of an R8NCF matrix.
-   **R8SP\_CG** uses the conjugate gradient method on an R8SP linear
    system.
-   **R8SP\_CHECK** checks that an R8SP matrix data structure is
    properly sorted.
-   **R8SP\_DIAGONAL** reorders an R8SP matrix so diagonal entries are
    first.
-   **R8SP\_DIF2** returns the DIF2 matrix in R8SP format.
-   **R8SP\_IJ\_TO\_K** seeks the compressed index of the (I,J) entry
    of A.
-   **R8SP\_INDICATOR** sets up an R8SP indicator matrix.
-   **R8SP\_JAC\_SL** solves an R8SP system using Jacobi iteration.
-   **R8SP\_MTV** multiplies a vector times an R8SP matrix.
-   **R8SP\_MV** multiplies an R8SP matrix times a vector.
-   **R8SP\_PRINT** prints an R8SP matrix.
-   **R8SP\_PRINT\_SOME** prints some of an R8SP matrix.
-   **R8SP\_RANDOM** sets a random R8SP matrix.
-   **R8SP\_READ** reads an R8SP matrix from a file.
-   **R8SP\_READ\_SIZE** reads the size of an R8SP matrix from a file.
-   **R8SP\_RES** computes the residual R = B-A\*X for R8SP matrices.
-   **R8SP\_TO\_R8GE** converts an R8SP matrix to an R8GE matrix.
-   **R8SP\_TO\_R8NCF** converts an R8SP matrix to an R8NCF matrix.
-   **R8SP\_WRITE** writes an R8SP matrix to a file.
-   **R8SP\_ZEROS** returns a zero R8SP matrix.
