...
//  Parameters:
//
//    Input, int SUGGP, the index of the first argument to be considered.
//
//    Input, int SUGG, the index of the first succeeding argument NOT 
//    to be considered.
//
{
  int c;
...
//
  ...
  for ( i = suggp; i < sugg; i++ )
  {
    ...
    if ( j == strlen ( my_argv[i] ) )
    {
      ...
      tl -= strlen ( my_argv[i] );
      do_search( starstring, my_argv[i] );
      for ( j = 0; j < strlen( my_argv[i] ); j++ )
      {
