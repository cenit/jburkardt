...
//    Input, int N, the number of data items.
//
//    Input, double LAT(N), LON(N), the latitude and longitude, 
//    in degrees.
//
//    Output, double LL_DEGREES_TO_DIST(N,N), the distance matrix.  Distances 
//    are measured in miles.
//
{
  double d;
  ...
  for ( i = 0; i < n; i++ )
  {
    ...
    for ( j = i + 1; j < n; j++ )
    {
      d = ll_degrees_to_distance_earth ( lat[i], lon[i], lat[j], lon[j] );
      dist_table[i+j*n] = d;
      dist_table[j+i*n] = d;
    }
  }
...
/*
  ...
  Parameters:
    ...
    Input, double LAT1, LON1, the latitude and longitude 
    of the first point.  

    Input, double LAT2, LON2, the latitude and longitude 
    of the second point. 

    Output, double LL_DEGREES_TO_DISTANCE_EARTH, the distance between 
    the points, in miles.
*/
{
  double dist;
...
/*
  ...
  Parameters:
    ...
    Output, double X[N], Y[N], the point coordinates, in miles.
*/
{
  int i;
  double phi;
